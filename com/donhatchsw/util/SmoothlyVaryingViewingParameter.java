...
// !!! THIS FILE WAS AUTOMATICALLY GENERATED BY javacpp; DO NOT EDIT !!!
// !!! THIS FILE WAS AUTOMATICALLY GENERATED BY javacpp; DO NOT EDIT !!!
// 102 # 1 "com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"
// 103 # 1 "<built-in>"
// 104 # 1 "<command-line>"
...
// 150 # 1 "com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"

// 152 # 1 "com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"
...
// XXX ... what a mess
// 166 # 4 "com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava" 2

/**
* A viewing parameter that always changes smoothly;
* that is, its position and velocity are continous,
* like an object with mass in the real world.
* You can set its target and it will take the quickest path
* to that target given a bound on acceleration;
* furthermore you can knock that target all over the place
* and the actual value will smoothly move towards it,
* without any instantaneous changes in velocity.
*
* TODO: make copy ctor instead of saving/restoring state... I think?
* TODO: get rid of the thousands of lines of mathematica crap that never worked I don't think
*
* BUG: demo3: TRY_TO_DO_IT_RIGHT: they all overshoot a bit it seems, with refTripTime=1?
* BUG: demo3: and in general quadratic seems to be the winner
* BUG: demo2: I think the more I zoom out, the more difficult it is to select things

* BUG: demo3: TRY_TO_DO_IT_RIGHT: they all overshoot a bit it seems, with refTripTime=1? ah it's just numerical instability I think
* BUG: demo3: and in general quadratic seems to be the winner? not sure. need to provide api to query settle time.

* TODO: time for a control panel I think
* TODO: ooh and they should look like the little spaceship from asteroids, pointing in dir of acceleration?
* TODO: demo2: lines connecting velocity dots with acceleration dots? not sure
* TODO: provide api for knowing how long it will take, and maybe even the catenary params if I can come up with a decent parametrization.  A full mathematical function would be nice!
* TODO: provide api for computing lots of times on planned trip at once, I think? and/or store catenary params so don't need to recompute if nothing changed... ? not sure. in any case there should be a way to make demo3 much faster than it is now, but with a mode in which it does all the computations it does now for exercise
* TODO: work on parametrization and robustness, especially on and near the x axis and especially on and near slack=0
* TODO: picking should only see things that are visible
* TODO: the following is resolved... can remove most of the discussion, but could leave in a description of how to convert from/to time-based bounce if desired
* TODO: Think about whether bounce should be in terms of time or distance.  The current code says "each piece takes sqrt(bounce) as much time as the previous",
*           i.e. it takes bounce as much distance as the previous. is that what we want? seems like for small bounce, distance-based will be really tiny, making it less manageable,
            say, with a slider.  I.e. the question is, if we have a bounce slider from 0 to 1,
            what does 1/2 mean?
            Does it mean each bounce takes 1/2 as much distance (and 1/sqrt(2) times as much time) as the previous?
            Or does it mean each bounce 1/2 as much time (and 1/4 as much distance) as the previous?

                                    distance-based interpretation                                          time-based interpretation
            slider: 1/sqrt(2)=.707  next bounce takes 1/sqrt(2) the distance and 1/sqrt(sqrt(2)) the time  next bounce takes 1/2 the distance and 1/sqrt(2) the time
            slider: 1/2             next bounce takes 1/2 the distance and 1/sqrt(2) the time              next bounce takes 1/4 the distance and 1/2 the time
            slider: 1/4             next bounce takes 1/4 the distance and 1/2 the time                    next bounce takes 1/16 the distance and 1/4 the time
            slider: 1/10            next bounce takes 1/10 the distance and 1/sqrt(10) the time            next bounce takes 1/100 the distance and 1/10 the time
            slider: 1/100           next bounce takes 1/100 the distance and 1/10 the time                 next bounce takes 1/10000 the distance and 1/100 the time

            Hmm. Advantage of distance based is it's easier to actually visually verify that it's working right.
            E.g. bounce=.5 means a 90 degree rotation will actually overshoot to exactly 135 degrees.
            Hmm I think maybe I like it that way.  E.g. intuitive for mc4d.
*/
public class SmoothlyVaryingViewingParameter
{
    /**
    * STRATEGY_SEPARABLE means handle each coordinate separately,
    * using the exact algorithm for 1 dimension,
    * with an attempt to scale the accelerations in each dimension
    * so that they all arrive at the same time.
    * STRATEGY_TWO_PIECE_QUADRATIC means use the general two-piece
    * quadratic solution.
    * WARNING: this setting makes mc4d hang, currently.
    * STRATEY_TRY_TO_DO_IT_RIGHT means use the optimal solution;
    * that is, the velocity traces out a catenary at unit speed
    * in velocity space.
    */
    public static final int STRATEGY_SEPARABLE = 0;
    public static final int STRATEGY_TWO_PIECE_QUADRATIC = 1;
    public static final int STRATEGY_TRY_TO_DO_IT_RIGHT = 2;
    public static final int STRATEGY_TRY_TO_DO_IT_RIGHT_WITH_SEPARABLE_BAILOUT = 3;
    public static final String strategyNames[] = {
        "STRATEGY_SEPARABLE",
        "STRATEGY_TWO_PIECE_QUADRATIC",
        "STRATEGY_TRY_TO_DO_IT_RIGHT",
    ...
    private double[] acceleration; // from most recent evolve if any
    private double[] observedAcceleration; // from begin and end velocities of most recent evolve
    private double[] targetPosition;

    /** Copy constructor. */
    public SmoothlyVaryingViewingParameter(SmoothlyVaryingViewingParameter from)
    {
        this.position = VecMath.copyvec(from.position);
        this.velocity = VecMath.copyvec(from.velocity);
        this.acceleration = VecMath.copyvec(from.acceleration);
        this.observedAcceleration = VecMath.copyvec(from.observedAcceleration);
        this.targetPosition = VecMath.copyvec(from.targetPosition);
    ...
    {
        this(new double[]{initialPosition});
    }
    /** Constructor. */
    public SmoothlyVaryingViewingParameter(double initialPosition[])
    {
        ...
        this.acceleration = VecMath.fillvec(this.position.length, Double.NaN);
        this.observedAcceleration = VecMath.fillvec(this.position.length, Double.NaN);
        this.targetPosition = VecMath.copyvec(initialPosition);
        this.strategy = (this.position.length==1 ? STRATEGY_SEPARABLE : STRATEGY_TRY_TO_DO_IT_RIGHT);
    }

    /** Sets the target value of this scalar variable. Equivalent to setTargetPosition(new double[]{targetPosition}); */
    public void setTargetPosition(double targetPosition)
    {
        setTargetPosition(new double[]{targetPosition});
    }
    ...
    /** Snaps to the specified position. Equivalent to setTargetPosition(targetPosition) followed by evolve(1.,0.,1.) (i.e. infinite power). */
    ...
    /** Sets current position, without changing velocity or target. If you want to do this, you are weird. */
    public void setPosition(double[] newPosition)
    {
        VecMath.copyvec(this.position, newPosition);
    }
    /** Sets current position (scalar version).  Equivalent to setPosition(new double[]{targetPosition}). */
    public void setPosition(double newPosition)
    {
        setPosition(new double[]{newPosition});
    }
    ...
    /** Gets the velocity of this scalar variable now.  Throws an IllegalArgumentException if this parameter was initialized as a vector of dimension other than 1. */
    public double getVelocity()
    {
        if (position.length != 1)
            throw new IllegalArgumentException("SmoothlyVaryingViewingParameter.getVelocity: no-arg version is only for scalars, this one is a vector of dimension "+position.length);
        return velocity[0];
    }
    /** Gets the velocity of this scalar variable now. */
    ...
    /** Gets the acceleration of this scalar variable now.  Throws an IllegalArgumentException if this parameter was initialized as a vector of dimension other than 1. */
    public double getAcceleration()
    {
        if (position.length != 1)
            throw new IllegalArgumentException("SmoothlyVaryingViewingParameter.getAcceleration: no-arg version is only for scalars, this one is a vector of dimension "+position.length);
        return acceleration[0];
    }
    /** Gets the acceleration of this scalar variable now. */
    public void getAcceleration(double acceleration[])
    {
        if (acceleration.length != this.acceleration.length)
            throw new IllegalArgumentException("SmoothlyVaryingViewingParameter.getAcceleration: return buffer size is "+acceleration.length+", expected "+this.acceleration.length+"");
        VecMath.copyvec(acceleration, this.acceleration);
    }
    /** Gets the observed acceleration of this scalar variable now; this is calculated from the velocity before and after the most recent evolve.  Throws an IllegalArgumentException if this parameter was initialized as a vector of dimension other than 1. */
    public double getObservedAcceleration()
    {
        if (position.length != 1)
            throw new IllegalArgumentException("SmoothlyVaryingViewingParameter.getObservedAcceleration: no-arg version is only for scalars, this one is a vector of dimension "+position.length);
        return observedAcceleration[0];
    }
    /** Gets the observed acceleration of this scalar variable now; this is calculated from te velocity before and after the most recent evolve. */
    public void getObservedAcceleration(double observedAcceleration[])
    {
        if (acceleration.length != this.acceleration.length)
            throw new IllegalArgumentException("SmoothlyVaryingViewingParameter.getObservedAcceleration: return buffer size is "+acceleration.length+", expected "+this.acceleration.length+"");
        VecMath.copyvec(observedAcceleration, this.observedAcceleration);
    }

    /** Sets the current velocity.  Throws an IllegalArgumentException if this parameter was initialized as a vector of dimension other than 1. */
    ...
    /** Sets the current velocity (scalar version). */
    public void setVelocity(double velocity)
    {
    ...
    /** Gets the target value of this scalar variable now. */
    public void getTargetPosition(double target[])
    {
        if (target.length != this.targetPosition.length)
            throw new IllegalArgumentException("SmoothlyVaryingViewingParameter.getTarget: return buffer size is "+target.length+", expected "+this.targetPosition.length+"");
        VecMath.copyvec(target, this.targetPosition);
    }
    ...
    */
    ...
    public static int catenaryVerboseLevel = 0;
    ...
    * <p>
    * @param distanceBasedBounce  Set this to a number from 0 to 1.  Setting it to a positive number causes the spring to be underdamped and so it will overshoot and oscillate as it converges on the target,
    * with each bounce going distanceBasedBounce as far as the previous one (and taking sqrt(distanceBasedBounce) as much time).
    */
    public void evolve(double referenceTripDistance,
                       double referenceTripTime,
                       double dt,
                       double distanceBasedBounce)
    {
        com.donhatchsw.compat.ArrayList newtonPath = null;
        this.evolve(referenceTripDistance,
                    referenceTripTime,
                    dt,
                    distanceBasedBounce,
                    verboseLevel, // the public static variable
                    newtonPath);
    }
    public void evolve(double referenceTripDistance,
                       double referenceTripTime,
                       ...
                       int verboseLevel,
                       com.donhatchsw.compat.ArrayList newtonPath) // if not null, x's and y's will be put in here, so caller can display it if desired
    {
        if (verboseLevel >= 1) System.out.println("    in evolve");



        double timeBasedBounce = Math.sqrt(distanceBasedBounce);


        if (verboseLevel >= 1)
        {
            {System.out.print("        "); System.out.println("strategyNames[strategy]" + " = " + (strategyNames[strategy]));};
            {System.out.print("        "); System.out.println("referenceTripDistance" + " = " + (referenceTripDistance));};
            {System.out.print("        "); System.out.println("referenceTripTime" + " = " + (referenceTripTime));};
            {System.out.print("        "); System.out.println("dt" + " = " + (dt));};
            {System.out.print("        "); System.out.println("distanceBasedBounce" + " = " + (distanceBasedBounce));};
            {System.out.print("        "); System.out.println("timeBasedBounce" + " = " + (timeBasedBounce));};
            System.out.println("      before:");
            System.out.println("        " + "this.velocity" + " = " + VecMath.toString(this.velocity));
            System.out.println("        " + "this.position" + " = " + VecMath.toString(this.position));
            System.out.println("        " + "this.targetPosition" + " = " + VecMath.toString(this.targetPosition));
        }

        if (!isMoving())
        {
            if (verboseLevel >= 1) System.out.println("        (not moving)");
            VecMath.fillvec(this.acceleration, 0.);
        }
        else
        {
            if (verboseLevel >= 2) System.out.println("      moving");
            //CHECK(false); // XXX coverage-- remove when hit
            if (referenceTripTime == 0.)
            {
                ...
                // isMoving() will subsequently return false.
                VecMath.copyvec(this.position, targetPosition);
                VecMath.zerovec(this.velocity);
                VecMath.fillvec(this.acceleration, Double.NaN); // actually infinite
                VecMath.fillvec(this.observedAcceleration, Double.NaN); // actually infinite
                if (verboseLevel >= 1) System.out.println("    out evolve (referenceTripTime is 0)");
                return;
            }
            //CHECK(false); // XXX coverage-- remove when hit

            // Save initial velocity in observedAcceleration...
            VecMath.copyvec(this.observedAcceleration, this.velocity);

            if (strategy == STRATEGY_SEPARABLE)
            {
                if (verboseLevel >= 2) System.out.println("      SEPARABLE");

                //CHECK(false); // XXX coverage-- remove when hit
                double p_and_v_and_a[] = new double[3]; // scratch
                ...
                if (position.length >= 2)
                {
                    if (true)
                    {
                        // previous way... left something to be desired.
                        // Hmm now that I think about it, this might actually
                        // be an extremely sane way of doing this??
                        // Way better than newton, I bet.  Maybe.
                        scales = VecMath.fillvec(position.length, 1./Math.sqrt(position.length));
                        if (verboseLevel >= 2) System.out.println("        " + "scales" + " = " + VecMath.toString(scales));
                        int nPreflights = 100;
                        for (int iPreflight = 0; iPreflight < nPreflights; ++iPreflight)
                        {
                            // XXX this results in bouncing, when close to diagonal but not exactly?? weird!
                            // Goal is to give each component an acceleration
                            // so that the norm of the vector of accelerations is the overall acceleration
                            // and each component arrives at the same time.
                            // Do a preflight to get estimates on the amount of work needed
                            // by each coordinate.
                            // The accelerations given to the various coords
                            // should be proportional to the squares of the times each thinks it will take to settle
                            // given equal portions. (Can repeat this any number of times if desired.)
                            for (int i = 0; i < position.length; ++i)
                            {
                                if (scales[i] != 0.)
                                {
                                    p_and_v_and_a[0] = this.position[i] - this.targetPosition[i];
                                    p_and_v_and_a[1] = this.velocity[i];
                                    p_and_v_and_a[2] = Double.NaN;
                                    double settleTime = update_p_and_v_and_a_1d(p_and_v_and_a, referenceTripDistance*scales[i], referenceTripTime, dt, distanceBasedBounce);
                                    // discard p_and_v_and_a result; we are only interested in settleTime, for this preflight
                                    if (verboseLevel >= 2) System.out.println("            i="+i+": settleTime="+settleTime);
                                    double correction;
                                    //correction = settleTime*settleTime; // I think that's theoretically right, and converges far faster in some cases, but causes wacko behavior in others
                                    //correction = Math.pow(settleTime, 1.5); // compromise
                                    correction = settleTime; // not as correct but more stable
                                    //correction = Math.sqrt(settleTime); // totally wrong

                                    // conservative for first 3, then less conservative for next 3,
                                    // then go for it.
                                    // Nope, just not stable, even with 100 iterations :-(
                                    if (false)
                                        correction = iPreflight < nPreflights/3 ? settleTime :
                                                     iPreflight < 2*nPreflights/3 ? Math.pow(settleTime, 1.5) :
                                                     settleTime*settleTime;
                                    if (false)
                                        correction = iPreflight < nPreflights/2 ? settleTime :
                                                     Math.pow(settleTime, 1.5);
                                    scales[i] *= correction;
                                }
                            }
                            //PRINTVEC(scales);
                            if (VecMath.normsqrd(scales) == 0.)
                            {
                                System.out.println("SURPRISE: SmoothlyVaryingViewingParameter strategy=separable: scales unexpectedly zero after "+(iPreflight+1)+"/"+nPreflights+" preflights: position="+VecMath.toString(position)+" targetPosition="+VecMath.toString(targetPosition)+" position-targetPosition="+VecMath.toString(VecMath.vmv(position,targetPosition))+" velocity="+VecMath.toString(velocity));
                                do { if (!((VecMath.normsqrd(scales))==(0.))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+399 +"): (" + "VecMath.normsqrd(scales)" + ")" + "==" + "(" + "0." + ") ("+(VecMath.normsqrd(scales))+" vs. "+(0.)+")"); } while (false);
                            }
                            VecMath.normalize(scales, scales);
                            if (verboseLevel >= 2) System.out.println("        " + "scales" + " = " + VecMath.toString(scales)); // XXX bleah, if using settleTime^2, when I print these out, they oscillate sometimes, e.g.
                            // scales = <0.900401218490184,0.43506050813811153>
                            // scales = <0.21387558341269755,0.9768609086353484>
                            // scales = <0.9985446970692868,0.05393039917158324>
                            // scales = <0.016514494716214887,0.9998636264332591>
                            // scales = <0.9999938828297847,0.00349775685413743>
                            // That's not the desired behavior!!
                            // and even if just using settleTime, they oscillate a little, which is weird.
                            // Clearest weirdness:  just click at 45 degrees, they all overshoot a little!  Wtf?
                            // That doesn't seem to make any sense-- if none of them would overshoot by itself,
                            // why would they overshoot together?
                            // Well, overshooting comes from when someone thinks they will have more power than they actually do,
                            // so they don't slow down as much as they should.
                            // Is that what's going on??
                            // Or... does one guy have more power at the beginning so it gets overconfident,
                            // but then that power gets taken away when it turns out the other guy needs it more?
                            //
                            // Or... do I have some logic bug that is making it overconfident by a factor of sqrt(2) or something?

                            // Get straight on this:
                            // With twice the acceleration, we go twice as far in a given time, right?
                        }
                        if (verboseLevel >= 2) System.out.println(""+"======");
                    }
                    if (false) // bleah! salvageable or not?
                    {
                        scales = new double[position.length];
                        double aMag = 4*referenceTripDistance/((referenceTripTime)*(referenceTripTime));
                        boolean bounceAware = false; // only works with false, currently
                        computeSeparableWeights(targetPosition, position, velocity, aMag, distanceBasedBounce, bounceAware, scales);
                ...
                for (int i = 0; i < position.length; ++i)
                {
                    ...
                    p_and_v_and_a[1] = this.velocity[i];
                    p_and_v_and_a[2] = Double.NaN;
                    double settleTime_ignored = update_p_and_v_and_a_1d(p_and_v_and_a,
                                                                        referenceTripDistance*(scales!=null ? scales[i] : 1.),
                                                                        referenceTripTime,
                                                                        dt,
                                                                        distanceBasedBounce);
                    this.position[i] = p_and_v_and_a[0] + this.targetPosition[i];
                    this.velocity[i] = p_and_v_and_a[1];
                    this.acceleration[i] = p_and_v_and_a[2];
                    do { if (!(!Double.isNaN(this.acceleration[i]))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+452 +"): " + "!Double.isNaN(this.acceleration[i])" + ""); } while (false);
                }
            }
            else if (strategy == STRATEGY_TRY_TO_DO_IT_RIGHT
                  || strategy == STRATEGY_TRY_TO_DO_IT_RIGHT_WITH_SEPARABLE_BAILOUT)
            {
                try
                {
                    if (verboseLevel >= 2) System.out.println("      "+strategyNames[strategy]);

                    double n = position.length;
                    double a = 4 * referenceTripDistance
                                 / ((referenceTripTime)*(referenceTripTime));
                    distanceBasedBounce = ((distanceBasedBounce)<=(1-1e-6)?(distanceBasedBounce):(1-1e-6)); // XXX should try to do something real in that case intead-- always pull directly towards the target.  what's the resulting path?  Did a sim about it... it's basically a spirograph drawing.  and asked a mathoverflow question about it.
                    // A is the overoptimistic acceleration
                    // such that, if we plan using it,
                    // we will overshoot by exactly distanceBasedBounce.
                    // Worked this out on paper...
                    double A = a * ((1.+distanceBasedBounce)/(1.-distanceBasedBounce));
                    if (verboseLevel >= 2) {System.out.print("        "); System.out.println("A" + " = " + (A));};

                    double velocityScaled[] = VecMath.copyvec(this.velocity);
                    double positionDisplacementScaled[] = VecMath.vmv(this.targetPosition, this.position);
                    // How do we deal with acceleration?
                    // For example if A=10, then we can scale velocity,position,targetPosition
                    // by 1/10, solve the problem (same dt), and then scale answer
                    // position and velocity by 10.
                    // XXX think about variations on this, changing dt
                    VecMath.vxs(velocityScaled, velocityScaled, 1./A);
                    VecMath.vxs(positionDisplacementScaled, positionDisplacementScaled, 1./A);

                    // Normalize velocityScaled and positionDisplacementScaled so that
                    // the max of their magnitudes is 1.
                    // If we multiply velocityScaled by something,
                    // we must multiply positionDisplacementScaled by something^2,
                    // and we must also divide dt by something.
                    // This is so we don't unnecessarily feed huge numbers to the catenary math,
                    // and also so that the next few assertions (about dot products of vectors)
                    // will be more sensible.
                    double normsqrdVelocity0Scaled = VecMath.normsqrd(velocityScaled);
                    double normPositionDisplacement = VecMath.norm(positionDisplacementScaled);
                    double problemSizeSqrd = Math.max(normsqrdVelocity0Scaled, normPositionDisplacement);
                    double problemSize = Math.sqrt(problemSizeSqrd);
                    boolean problemSizeScaleFlag = true;
                    if (problemSizeScaleFlag)
                    {
                        VecMath.vxs(velocityScaled, velocityScaled, 1./problemSize);
                        VecMath.vxs(positionDisplacementScaled, positionDisplacementScaled, 1./problemSizeSqrd);
                        do { if (!((Math.max(VecMath.norm(velocityScaled), VecMath.norm(positionDisplacementScaled)))==(1.)||Math.abs((Math.max(VecMath.norm(velocityScaled), VecMath.norm(positionDisplacementScaled)))-(1.))<=1e-12)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+500 +"): " +"(" + "Math.max(VecMath.norm(velocityScaled), VecMath.norm(positionDisplacementScaled))" + ")==(" + "1." + ")+-" + "1e-12" + "" +" ("+(Math.max(VecMath.norm(velocityScaled), VecMath.norm(positionDisplacementScaled)))+" vs. "+(1.)+" +- "+(1e-12)+")" +" (error = "+((Math.max(VecMath.norm(velocityScaled), VecMath.norm(positionDisplacementScaled)))-(1.))+")" ); } while (false);
                    }

                    // We want to work in 2 dimensions.
                    // So find two orthogonal vectors spanning the space
                    // containing 0, velocityScaled, and positionDisplacementScaled.
                    // With respect to this basis, we want velocityScaled to go on the negative x axis
                    // and positionDisplacement to be in the 3rd or 4th quadrant.
                    if (verboseLevel >= 2) System.out.println("        " + "velocityScaled" + " = " + VecMath.toString(velocityScaled));
                    if (verboseLevel >= 2) System.out.println("        " + "positionDisplacementScaled" + " = " + VecMath.toString(positionDisplacementScaled));
                    double basis[][];
                    if (n == 1)
                        basis = new double[][] {{velocityScaled[0]<0.?-1.:1.},{0}}; // hack, just so we can convert from 1d to 2d and back
                    else
                        basis = VecMath.extendAndOrthogonalize(2, 2, new double[][]{
                            velocityScaled,
                            positionDisplacementScaled,
                        });
                    VecMath.sxm(basis, -1., basis);
                    if (verboseLevel >= 3) System.out.println("        " + "basis" + " =\n" + VecMath.toString(basis));
                    if (verboseLevel >= 3) {System.out.print("        "); System.out.println("VecMath.dot(velocityScaled, basis[0])" + " = " + (VecMath.dot(velocityScaled, basis[0])));};
                    if (verboseLevel >= 3) {System.out.print("        "); System.out.println("VecMath.dot(positionDisplacementScaled, basis[1])" + " = " + (VecMath.dot(positionDisplacementScaled, basis[1])));};
                    do { if (!(VecMath.dot(velocityScaled, basis[0]) <= 0.)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+522 +"): " + "VecMath.dot(velocityScaled, basis[0]) <= 0." + ""); } while (false);

                    double tol = 1e-12;
                    // XXX TODO: actually the following isn't quite the right condition...
                    // should do something based on condition number of matrix, or something?
                    // hmm https://answers.yahoo.com/question/index?qid=20111206034618AAI8OvC
                    // says condition number is ||M|| ||M^-1|| where ||.|| can be any of a number of norms:
                    //  1-norm = max value of sum of abs values of col elements
                    //  infinity-norm = max value of sum of absolute values of row elements.
                    //  2-norm = square root of largest eigenvalue of A^T A -- necessarily nonnegative
                    // but really can't we do a decent estimate without inverting?
                    // just from M M^T, I think, i.e. lengths squared and dot prods
                    // of the two vectors.
                    // Also see: http://www.maths.manchester.ac.uk/~higham/talks/claode13.pdf
                    //  || a b c d || || d -b -c a || / (ad-bc)^2
                    // So that's: (max row sum) * (max col sum) / det^2
                    //
                    // Look like good refs:
                    //     "Some remarks on matrix norms, condition numbers, and error estimates for linear equations"
                    //     http://ac.els-cdn.com/0024379583901301/1-s2.0-0024379583901301-main.pdf?_tid=52fc7168-2bf6-11e5-8d32-00000aacb35f&acdnat=1437077446_c65e944cfa64748fcd6662cefaa66868
                    //     "Estimating matrix condition numbers" (suggestions based on LU)
                    //     http://www.cs.umd.edu/~oleary/reprints/j07.pdf
                    do { if (!(VecMath.dot(positionDisplacementScaled, basis[1]) <= tol)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+544 +"): " + "VecMath.dot(positionDisplacementScaled, basis[1]) <= tol" + ""); } while (false)
                               ;

                    double v[/*2*/] = VecMath.mxv(basis, velocityScaled); // from nd to 2d space
                    double p[/*2*/] = VecMath.mxv(basis, positionDisplacementScaled);// from nd to 2d space
                    // v should be on non-negative x axis,
                    // p should be in the 3rd or 4th quadrant
                    if (verboseLevel >= 2) System.out.println("        " + "v" + " = " + VecMath.toString(v));
                    if (verboseLevel >= 2) System.out.println("        " + "p" + " = " + VecMath.toString(p));
                    do { if (!(v[0] <= 0.)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+553 +"): " + "v[0] <= 0." + ""); } while (false); // ok to make this one exact
                    do { if (!(((((v[1])-(0.)) <= (tol)) && (((0.)-(v[1])) <= (tol))))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+554 +"): " + "EQ(v[1], 0., tol)" + ""); } while (false);
                    if (!(p[1] <= tol))
                    {
                        System.out.println("        " + "v" + " = " + VecMath.toString(v));
                        System.out.println("        " + "p" + " = " + VecMath.toString(p));
                    }
                    do { if (!(p[1] <= tol)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+560 +"): " + "p[1] <= tol" + ""); } while (false);

                    // We want to find the catenary with end points v,(0,0)
                    // and integral p.
                    final CatenaryRotated catenaryRotated = new CatenaryRotated();
                    catenaryRotated.fitFromMoment(
                        v[0], v[1], // v0; happens to be on -x axis now, but doesn't need to be
                        0., 0., // v1
                        p[0], p[1], // desired integral
                        null, // newtonPathOut
                        catenaryVerboseLevel);

                    if (false)
                    {
                        // sanity check! XXX do this? how much of it?
                        System.out.println(""+"=============");
                        double pos[] = new double[2];
                        double vel[] = new double[2];
                        double acc[] = new double[2];
                        // vel at t0 should be v
                        catenaryRotated.evaluate(catenaryRotated.t0, vel);
                        System.out.println("        " + "vel" + " = " + VecMath.toString(vel));
                        System.out.println("        " + "v" + " = " + VecMath.toString(v));
                        do { if (!(VecMath.distsqrd(vel, v) <= ((tol)*(tol)))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+583 +"): " + "VecMath.distsqrd(vel, v) <= SQR(tol)" + ""); } while (false);
                        System.out.println(""+"---");
                        // velocity at t1 should be 0
                        catenaryRotated.evaluate(catenaryRotated.t1, vel);
                        System.out.println("        " + "vel" + " = " + VecMath.toString(vel));
                        System.out.println("        " + "VecMath.zerovec(2)" + " = " + VecMath.toString(VecMath.zerovec(2)));
                        do { if (!(VecMath.normsqrd(vel) <= ((tol)*(tol)))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+589 +"): " + "VecMath.normsqrd(vel) <= SQR(tol)" + ""); } while (false);
                        System.out.println(""+"---");
                        // integral from t0 to t1 should be p
                        catenaryRotated.integrate(catenaryRotated.t0, catenaryRotated.t1, pos);
                        System.out.println("        " + "pos" + " = " + VecMath.toString(pos));
                        System.out.println("        " + "p" + " = " + VecMath.toString(p));
                        do { if (!(VecMath.distsqrd(pos, p) <= ((tol)*(tol)))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+595 +"): " + "VecMath.distsqrd(pos, p) <= SQR(tol)" + ""); } while (false);
                        System.out.println(""+"---");
                        // acceleration at t0 and t1 should have magnitude 1
                        catenaryRotated.differentiate(catenaryRotated.t0, acc);
                        {System.out.print("        "); System.out.println("VecMath.norm(acc)" + " = " + (VecMath.norm(acc)));};
                        {System.out.print("        "); System.out.println("1." + " = " + (1.));};
                        do { if (!(Math.abs(VecMath.norm(acc) - 1.) <= tol)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+601 +"): " + "Math.abs(VecMath.norm(acc) - 1.) <= tol" + ""); } while (false);
                        System.out.println(""+"---");
                        catenaryRotated.differentiate(catenaryRotated.t1, acc);
                        {System.out.print("        "); System.out.println("VecMath.norm(acc)" + " = " + (VecMath.norm(acc)));};
                        {System.out.print("        "); System.out.println("1." + " = " + (1.));};
                        do { if (!(Math.abs(VecMath.norm(acc) - 1.) <= tol)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+606 +"): " + "Math.abs(VecMath.norm(acc) - 1.) <= tol" + ""); } while (false);
                        System.out.println(""+"---");

                        System.out.println(""+"=============");
                    }

                    double maxArcLength = dt / (problemSizeScaleFlag ? problemSize : 1.);
                    double t1 = ((catenaryRotated.t0+maxArcLength)<=(catenaryRotated.t1)?(catenaryRotated.t0+maxArcLength):(catenaryRotated.t1));

                    double pos[] = new double[2];
                    double vel[] = new double[2];
                    double acc[] = new double[2];
                    // compute new position displacement
                    catenaryRotated.integrate(catenaryRotated.t0, t1, pos);
                    // compute new velocity
                    catenaryRotated.evaluate(t1, vel);
                    // compute new acceleration
                    catenaryRotated.differentiate(t1, acc);

                    // Convert new velocityScaled and positionDisplacementScaled from
                    // 2d back to original space
                    VecMath.vxm(velocityScaled, vel, basis);
                    VecMath.vxm(positionDisplacementScaled, pos, basis);
                    VecMath.vxm(this.acceleration, acc, basis); // still unit length

                    if (problemSizeScaleFlag)
                    {
                        // Undo the problem size scale
                        VecMath.vxs(velocityScaled, velocityScaled, problemSize);
                        VecMath.vxs(positionDisplacementScaled, positionDisplacementScaled, problemSizeSqrd);
                        // this.acceleration still unit length
                    }

                    if (verboseLevel >= 2)
                    {
                        {System.out.print("        "); System.out.println("catenaryRotated" + " = " + (catenaryRotated));};
                        System.out.println("        basis = "+Arrays.toStringCompact(basis));
                        System.out.println(""+"      old:");
                        System.out.println("        " + "velocity" + " = " + VecMath.toString(velocity));
                        System.out.println("        " + "position" + " = " + VecMath.toString(position));
                    }

                    // Undo the acceleraton scale, back into member vars.
                    VecMath.vxs(this.acceleration, this.acceleration, A); // now length A XXX what? should be a. but I think others are similarly wrong?  well it's known that bounce isn't working for this case yet anyway...
                    VecMath.vxs(this.velocity, velocityScaled, A);
                    VecMath.vxs(positionDisplacementScaled, positionDisplacementScaled, A);
                    VecMath.vpv(this.position, this.position, positionDisplacementScaled);

                    // We've set the new position and velocity and acceleration.
                    // Done with this timestep.

                    if (verboseLevel >= 2)
                    {
                        System.out.println(""+"      new:");
                        System.out.println("        " + "this.acceleration" + " = " + VecMath.toString(this.acceleration));
                        System.out.println("        " + "this.velocity" + " = " + VecMath.toString(this.velocity));
                        System.out.println("        " + "this.position" + " = " + VecMath.toString(this.position));
                    }

                    if (t1 == catenaryRotated.t1)
                    {
                        // We are done with entire journey!
                        // Position should be target and velocity should be
                        // zero, but they may be slightly different due to
                        // floating point roundoff error.
                        // Set them to exactly what they should be,
                        // so that isMoving()'s criteria will be met.
                        // TODO: sanity check mode? what should be done along these lines?
                        if (verboseLevel >= 2) System.out.println("Done with entire journey because t1="+t1+" == catenary.t1");
                        VecMath.zerovec(this.acceleration);
                        VecMath.zerovec(this.velocity);
                        VecMath.copyvec(this.position, this.targetPosition);
                        if (verboseLevel >= 2)
                        {
                            System.out.println(""+"      adjusted new:");
                            System.out.println("        " + "acceleration" + " = " + VecMath.toString(acceleration));
                            System.out.println("        " + "velocity" + " = " + VecMath.toString(velocity));
                            System.out.println("        " + "position" + " = " + VecMath.toString(position));
                        }
                    }
                ...
                {
                    if (strategy != STRATEGY_TRY_TO_DO_IT_RIGHT_WITH_SEPARABLE_BAILOUT)
                    {
                        // no bail-out: proceed with throw.
                        // Note, in >=1.7 we could re-throw the throwable, but in <=1.6 that gives
                        // "error: unreported exception Throwable; must be caught or declared to be thrown"
                        // so we do this instead, as suggested in:
                        // http://stackoverflow.com/questions/11707656/methodhandle-must-be-caught-or-declared-to-be-thrown-why-this-error#answer-11707695
                        throw new AssertionError(throwable);
                    }

                    if (true)
                    {
                        {System.out.print("        "); System.out.println("throwable" + " = " + (throwable));};
                        throwable.printStackTrace();
                        System.out.println("OH NO! SOMETHING AWFUL HAPPENED!");
                        ...
                        System.out.println("Falling back on simple separable method, although not really so simple");
                    }
                    else
                    {
                        System.out.print("!");
                    ...
                    try
                    {
                        strategy = STRATEGY_SEPARABLE;
                        evolve(referenceTripDistance,
                               referenceTripTime,
                               dt,
                               distanceBasedBounce,
                               verboseLevel,
                    ...
                    if (verboseLevel >= 1)
                    {
                        System.out.println("    out evolve (outer STRATEGY_TRY_TO_DO_IT_RIGHT_WITH_SEPARABLE_BAILOUT bail-out)");
                    }
                    return;
                }
            } // if strategy == STRATEGY_TRY_TO_DO_IT_RIGHT or STRATEGY_TRY_TO_DO_IT_RIGHT_WITH_SEPARABLE_BAILOUT
            else if (strategy == STRATEGY_TWO_PIECE_QUADRATIC)
            {
                if (verboseLevel >= 2) System.out.println("      TWO_PIECE_QUADRATIC");
                //
                // Do the two-piece quadratic thing.
                // Acceleration is two-piece constant,
                // velocity is two-piece linear,
                // position is two-piece quadratic,
                //

                //CHECK(false); // XXX coverage-- remove when hit
                double a = 4 * referenceTripDistance
                             / ((referenceTripTime)*(referenceTripTime));
                distanceBasedBounce = ((distanceBasedBounce)<=(1-1e-6)?(distanceBasedBounce):(1-1e-6)); // XXX should try to do something real in that case intead-- always pull directly towards the target.  what's the resulting path?  I don't know
                double vKnee[] = new double[velocity.length];
                if (distanceBasedBounce < 1.)
                {
                    //CHECK(false); // XXX coverage-- remove when hit
                    // A is the overoptimistic acceleration
                    // such that, if we plan using it,
                ...
                {
                    do { if (!(false)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+772 +"): " + "false" + ""); } while (false); // XXX coverage-- remove when hit
                    // A = infinity, vKnee should have infinite magnitude in the direction of the desired position change, I think
                    do { if (!(false)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+774 +"): " + "false" + ""); } while (false);
                }
                ...
                if (dt <= timeBeforeKnee)
                {
                    ...
                    VecMath.vxs(acceleration,
                                beforeKneeVelocityDirection,
                                a);
                }
                else // dt > timeBeforeKnee
                {
                    ...
                    if (dt >= timeBeforeKnee + timeAfterKnee)
                    {
                        ...
                        VecMath.zerovec(acceleration);
                        if (verboseLevel >= 1) System.out.println("    out evolve (dt >= timeBeforeKnee + timeAfterKnee)");
                        return;
                    }
                    ...
                    VecMath.vxs(acceleration,
                                afterKneeVelocityDirection,
                                a);
                }
                //CHECK(false); // XXX coverage-- remove when hit
            } // if strategy == STRATEGY_TWO_PIECE_QUADRATIC
            else
            {
                // strategy isn't one of the possible strategies
                do { if (!(false)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+837 +"): " + "false" + ""); } while (false);
            }

            VecMath.vmv(this.observedAcceleration,
                        this.velocity, this.observedAcceleration);
            VecMath.vxs(this.observedAcceleration, this.observedAcceleration, 1./dt);
        ...
        if (verboseLevel >= 1) System.out.println("    out evolve");
    } // evolve
    ...
    public boolean isMoving()
    {
        return !VecMath.equalsExactly(position, targetPosition)
            || !VecMath.isZeroExactly(velocity);
    }
    ...
    {
        if (verboseLevel >= 1) System.out.println("        in computeVelocityKnee(p="+Arrays.toStringCompact(p)+", v="+Arrays.toStringCompact(v)+", a="+a+")");
        //CHECK(false); // XXX coverage-- remove when hit
        //
        ...
        do { if (!(Math.abs(VecMath.dot(uDir,vDir)) < 1e-15)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+950 +"): " + "Math.abs(VecMath.dot(uDir,vDir)) < 1e-15" + ""); } while (false);
        ...
        if (true)
        {
            double c0 = 16*((yDesiredIntegral)*(yDesiredIntegral));
            double c2 = 1 - 16*((xDesiredIntegral)*(xDesiredIntegral)) - c0;
        ...
        else // old way
        {
            ...
            for (;; iIteration++)
            {
                ...
                double signedDistanceFromEllipse = r==0. ? ((x)*(x))+((y)*(y))
                                                 : ((x*R)*(x*R)) + ((y*r)*(y*r)) - ((r*R)*(r*R));
        ...
        else
        {
            if (false)
            {
                ...
                xKnee = (-1
                       ...
                       - Math.sqrt(((1 + slack - 2*xDesiredIntegral)*(1 + slack - 2*xDesiredIntegral))
                ...
                //
                if (slack < 1e-6)
                    xKnee = ((xKnee)<=(-slack*.5)?(-slack*.5):(xKnee)>=(slack*.5)?(slack*.5):(xKnee));
            }
            else
            {
                ...
                do { if (!(yRadius != 0.)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1199 +"): " + "yRadius != 0." + ""); } while (false); // since slack != 0.
        ...
        if (verboseLevel >= 1) System.out.println("        out computeVelocityKnee, returning velocityKnee = "+Arrays.toStringCompact(velocityKnee));
        ...
        //
        {
            ...
            double tol = 1e-6;
            do { if (!(error <= tol)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1237 +"): " + "error <= tol" + ""); } while (false); // XXX should be relative
        }

    } // computeVelocityKnee
// 1404 # 1361 "com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"
    // utility function, evolves a single coordinate
    // towards zero position and velocity.
    // returns the amount of time, starting at the original time, needed to settle.
    private static double update_p_and_v_and_a_1d(double p_and_v_and_a[/*3*/],
    ...
    {
        boolean debug = false;
        if (debug) System.out.println("in update_p_and_v_and_a_1d");
        do { if (!(distanceBasedBounce >= 0)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1372 +"): " + "distanceBasedBounce >= 0" + ""); } while (false); // XXX make this a bad param exception or something

        if (referenceTripDistance == 0.)
        {
            // no acceleration, don't move
            p_and_v_and_a[2] = 0.;
            return Double.POSITIVE_INFINITY; // XXX or zero if there already? not sure
        }
        if (referenceTripTime == 0.)
        {
        ...
        double aMag = 4 * referenceTripDistance
                        / ((referenceTripTime)*(referenceTripTime));
        // XXX um, what if referenceTripTime is 0? shouldn't this get the code from the old implementation?
        double p = p_and_v_and_a[0];
        double v = p_and_v_and_a[1];
        double a = Double.NaN; // old acceleration from p_and_v_and_a irrelevant here
        ...
        double t0, p0, a0;
        if (p*v <= 0 &&
            still_p_after/still_p_before > -distanceBasedBounce) // XXX FIX THIS-- I think there is danger of a zero-divide here?
        {
        ...
        double p1 = -distanceBasedBounce*p0;
        double a1 = -a0;
        double t1 = t0 + Math.sqrt((Math.abs(p1-p0)/referenceTripDistance))
                       * referenceTripTime;

        // Each piece takes sqrt(distanceBasedBounce) as much time as the previous one.
        // So the time at which we will settle is:
        //     t0 + (t1-t0)*(1 + distanceBasedBounce^(1/2) + distanceBasedBounce + distanceBasedBounce^(3/2) + ...)
        //   = t0 + (t1-t0)/(1-sqrt(distanceBasedBounce))
        // Check whether we are being asked to evolve to or beyond
        // the settling time, i.e. whether
        //     dt >= t0 + (t1-t0)/(1-sqrt(distanceBasedBounce))
        // Don't do that division explicitly (in case distanceBasedBounce is very close
        // to 1); instead, check whether:
        //     (dt-t0)*(1-sqrt(distanceBasedBounce)) >= t1-t0

        // actually do the division after all, for the return value.
        double settleTime = distanceBasedBounce==1. ? Double.POSITIVE_INFINITY : t0 + (t1-t0)/(1-Math.sqrt(distanceBasedBounce));

        if ((dt-t0)*(1-Math.sqrt(distanceBasedBounce)) >= t1-t0)
        {
            // Settled
        ...
        else
        {
            ...
            while (dt > t_i)
            {
                ...
                a_iminus1 = a_i;
                i++;
                p_i = -distanceBasedBounce * p_iminus1;
                a_i = -a_iminus1;
                ...
                if (i > 1000*1000)
                {
                    ...
                    do { if (!(false)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1508 +"): " + "false" + ""); } while (false); // coverage
                    p_and_v_and_a[0] = 0.;
                    p_and_v_and_a[1] = 0.;
                    p_and_v_and_a[2] = 0.;
                    return settleTime;
            ...
            if (frac < .5)
            {
                ...
                p = p_iminus1 + .5*a_iminus1*((dt-t_iminus1)*(dt-t_iminus1));
            ...
            else
            {
                ...
                a = a_i;
                v = (t_i-dt) * -a_i;
                p = p_i - .5* -a_i *((t_i-dt)*(t_i-dt));
            }
        }
        p_and_v_and_a[0] = p;
        p_and_v_and_a[1] = v;
        p_and_v_and_a[2] = a;
        if (debug) System.out.println("out update_p_and_v_1d, returning settleTime="+settleTime);
        return settleTime;
    } // private update_p_and_v_1d

    private double settleTime1d(double p, double v, double aMag, double bounce, boolean bounceAware)
    {
        // The math below assumes p is nonnegative.
        // If not, just negate the problem.
        if (p < 0.)
        {
            p = -p;
            v = -v;
        ...
        if (!bounceAware)
        {
            // Just compute answer using overoptimistic acceleration
            double A = aMag * (1.+bounce)/(1.-bounce);
            if (v < 0. && ((v*v)/(2.*A) - p) > 0.)
            {
                // Positive then negative acceleration.
                // Positive (decelerate) to still point which is past origin,
                // then canonical journey (still positive, then negative) to origin.
                answer = -v/A + 2.*Math.sqrt((v*v/(2.*A) - p)/A);
            }
            else
            {
                // Negative then positive acceleration.
                // If v>=0: negative (decelerate) to still point,
                // then canonical journey (still negative, then positive) to target.
                // And same formula works if v not too negative.
                answer = 2.*Math.sqrt((v*v/(2.*A) + p)/A) + v/A;
            }
        }
        else
        {
            do { if (!(false)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1579 +"): " + "false" + ""); } while (false); // XXX implement me!
        }
        return answer;
    } // settleTime

    // Compute acceleration weights for the components of acceleration
    // so that all components will arrive at the same time.
    // If bounceAware: equalize actual infinite-sum settle times with original accels
    // If not bounceAware: equalize simple settle times with overoptimistic accels
    private void computeSeparableWeights(double pTarget[], double p[], double v[], double aMag,
                                         double bounce, boolean bounceAware,
                                         double answerWeights[])
    {
        final int verboseLevel = 0;
        if (verboseLevel >= 1) System.out.println("            in computeSeparableWeights(p="+VecMath.toString(p)+", v="+VecMath.toString(v)+", aMag="+aMag+", bounce="+bounce+", bounceAware="+bounceAware);
        final int n = p.length;
        do { if (!((v.length)==(n))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1595 +"): (" + "v.length" + ")" + "==" + "(" + "n" + ") ("+(v.length)+" vs. "+(n)+")"); } while (false); // precondition
        do { if (!((answerWeights.length)==(n))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1596 +"): (" + "answerWeights.length" + ")" + "==" + "(" + "n" + ") ("+(answerWeights.length)+" vs. "+(n)+")"); } while (false); // precondition
        if (n == 0) return;
        if (n == 1) {answerWeights[0] = 1.; return;}
        do { if (!(n >= 2)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1599 +"): " + "n >= 2" + ""); } while (false); // logical assertion

        // Solve for barycentric weights for squares of the components.
        final double pTargetFinal[] = pTarget;
        final double pFinal[] = p;
        final double vFinal[] = v;
        final double aMagFinal = aMag;
        final double bounceFinal = bounce;
        final boolean bounceAwareFinal = bounceAware;
        NewtonSolver.Fun fun = new NewtonSolver.Fun(n-1) {
            @Override public void f(double barys[], double answerDiffs[])
            {
                if (verboseLevel >= 3) System.out.println("                in f(barys = "+Arrays.toStringCompact(barys)+")");
                double lastBary = 1. - VecMath.sum(barys);
                double lastSettleTime = Double.NaN; // to shut up compiler
                for (int i = 0; i < n; ++i)
                {
                    double bary = i==n-1 ? lastBary : barys[i];
                    if (bary < 0.) {System.out.print("        "); System.out.println("bary" + " = " + (bary));};
                    do { if (!(bary >= 0.)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1618 +"): " + "bary >= 0." + ""); } while (false);
                    double settleTime = settleTime1d(pFinal[i]-pTargetFinal[i], vFinal[i], aMagFinal*Math.sqrt(bary), bounceFinal, bounceAwareFinal);
                    if (i == n-1)
                        lastSettleTime = settleTime;
                    else
                        answerDiffs[i] = settleTime;
                }
                if (verboseLevel >= 2) System.out.println("                  settleTimes = "+Arrays.toStringCompact(answerDiffs)+" and finally "+lastSettleTime);
                for (int i = 0; i < answerDiffs.length; ++i)
                    answerDiffs[i] -= lastSettleTime;
                if (verboseLevel == 2) System.out.println("                f(barys = "+Arrays.toStringCompact(barys)+") = answerDiffs="+Arrays.toStringCompact(answerDiffs));
                if (verboseLevel >= 3) System.out.println("                out f(barys = "+Arrays.toStringCompact(barys)+"), returning answerDiffs="+Arrays.toStringCompact(answerDiffs));
            }
        };
        double barys[] = VecMath.fillvec(position.length-1, 1./position.length); // initial guess
        int minIterations = 10;
        int maxIterations = 1000;
        double zeros[] = VecMath.fillvec(position.length-1, 0.);
        boolean adaptiveFlag = false; // XXX true behaves a bit, but can make things really slow!  should look into why it's unstable in the first place instead
        NewtonSolver.solve(barys, zeros, fun, minIterations, maxIterations, adaptiveFlag);
        for (int i = 0; i < n; ++i)
        {
            double bary = i==n-1 ? 1.-VecMath.sum(barys) : barys[i];
            answerWeights[i] = Math.sqrt(bary);
        }
        if (verboseLevel >= 1) System.out.println("            out computeSeparableWeights(p="+VecMath.toString(p)+", v="+VecMath.toString(v)+", aMag="+aMag+", bounce="+bounce+", bounceAware="+bounceAware+", returning "+VecMath.toString(answerWeights));
    } // computeSeparableWeights
    ...
    private static void confidenceTests()
    {
        ...
        if (true) // actually get other stuff working first!
        {
            // Try some things for which "OH NO! SOMETHING AWFUL HAPPENED!" in demo3,
            // or for which it flipped out.

            if (true)
            {
                // did not flip out
                int strategy = STRATEGY_TRY_TO_DO_IT_RIGHT; // no bailout!
                double referenceTripDistance = 0.01;
                double referenceTripTime = 1.0;
                double dt = 172.0;
                double distanceBasedBounce = 0.0;
                double initialVelocity[] = {-6.350325358350921,-0.2328047333925734};
                double initialPosition[] = {1051.1052838738922,209.27970728812923};
                double targetPosition[] = {486.0,169.0};
                SmoothlyVaryingViewingParameter particle = new SmoothlyVaryingViewingParameter(initialPosition);
                particle.strategy = strategy;
                particle.setVelocity(initialVelocity);
                particle.setTargetPosition(targetPosition);
                int verboseLevel = 2;
                particle.evolve(referenceTripDistance,referenceTripTime, dt, distanceBasedBounce, verboseLevel, null);
            }
            if (true)
            {
                // almost same, flipped out. that is, evolve from a slightly advanced position and it goes a bit wild-- ends at 364.9229121205566,374.91442946180166
                // when it should be ending at the target or thereabouts
                int strategy = STRATEGY_TRY_TO_DO_IT_RIGHT; // no bailout!
                double referenceTripDistance = 0.01;
                double referenceTripTime = 1.0;
                double dt = 196.;
                double distanceBasedBounce = 0.0;
                double initialVelocity[] = {-6.388642131165421,-0.24427367072094044};
                double initialPosition[] = {1044.735755646969,209.04131632665735};
                double targetPosition[] = {486.0,169.0};
                SmoothlyVaryingViewingParameter particle = new SmoothlyVaryingViewingParameter(initialPosition);
                particle.strategy = strategy;
                particle.setVelocity(initialVelocity);
        ...
        if (true)
        {
            System.out.println("        Verifying that all three strategies give the same answer when given a 1d problem");
            System.out.println("        XXX except TWO_PIECE_QUADRATIC hangs on 1d problem");
            int strategies[] = {
                STRATEGY_SEPARABLE,
                //STRATEGY_TWO_PIECE_QUADRATIC, // TODO: make this work!
                //STRATEGY_TRY_TO_DO_IT_RIGHT,
                STRATEGY_TRY_TO_DO_IT_RIGHT_WITH_SEPARABLE_BAILOUT, // don't test this one-- it masks errors!
            };
            SmoothlyVaryingViewingParameter particles[][] = new SmoothlyVaryingViewingParameter[2][strategies.length];
            for (int i = 0; i < 2; ++i)
            for (int j = 0; j < strategies.length; ++j)
            {
                int strategy = strategies[j];
                particles[i][j] = i==0 ? new SmoothlyVaryingViewingParameter(0.)
                                                    : new SmoothlyVaryingViewingParameter(new double[2]);
                particles[i][j].strategy = strategy;
            }

            // acceleration = 4*d/t^2
            //double referenceTripDistance = .01;
            //double referenceTripDistance = .1;
            //double referenceTripDistance = .5; // for acceleration=1
            double referenceTripDistance = 5.;
            double referenceTripTime = 1.;
            double dt = 1.;
            double distanceBasedBounce = 0.;
            double vStart = 0.;
            //double pStart = 300.;
            //double targetPosition = 200.;
            double pStart = 100.;
            ...
            {
                int strategy = strategies[j];
                SmoothlyVaryingViewingParameter particle = particles[i][j];
                if (i == 0)
                {
                    particle.setVelocity(vStart);
                    particle.setTargetPosition(targetPosition);
                    particle.setPosition(pStart);
                }
                else
                {
                    particle.setVelocity(new double[] {vStart, 0.});
                    particle.setTargetPosition(new double[] {targetPosition, 0.});
                    particle.setPosition(new double[] {pStart, 0.});
                }
            }
            ...
            while (true)
            {
                System.out.println("            nItersDone = "+nItersDone);
                double ps[][][] = new double[2][strategies.length][];
                double vs[][][] = new double[2][strategies.length][];

                for (int i = 0; i < 2; ++i)
                for (int j = 0; j < strategies.length; ++j)
                {
                    ps[i][j] = new double[i==0?1:2];
                    vs[i][j] = new double[i==0?1:2];
                    particles[i][j].get(ps[i][j]);
                    particles[i][j].getVelocity(vs[i][j]);
                }

                for (int i = 0; i < 2; ++i)
                for (int j = 0; j < strategies.length; ++j)
                    System.out.println("                ps["+i+"]["+j+"] = "+VecMath.toString(ps[i][j]));
                for (int i = 0; i < 2; ++i)
                for (int j = 0; j < strategies.length; ++j)
                    System.out.println("                vs["+i+"]["+j+"] = "+VecMath.toString(ps[i][j]));
                for (int i = 0; i < 2; ++i)
                for (int j = 0; j < strategies.length; ++j)
                    System.out.println("                particles["+i+"]["+j+"].isMoving() = "+particles[i][j].isMoving());

                for (int i = 0; i < 2; ++i)
                for (int j = 0; j < strategies.length; ++j)
                {
                    //double tol = 1e-12;
                    //double tol = 1e-6; // XXX too coarse!
                    double tol = 1e-4; // XXX too coarse!
                    do { if (!((ps[i][j][0])==(ps[0][0][0])||Math.abs((ps[i][j][0])-(ps[0][0][0]))<=tol)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1797 +"): " +"(" + "ps[i][j][0]" + ")==(" + "ps[0][0][0]" + ")+-" + "tol" + "" +" ("+(ps[i][j][0])+" vs. "+(ps[0][0][0])+" +- "+(tol)+")" +" (error = "+((ps[i][j][0])-(ps[0][0][0]))+")" ); } while (false);
                    do { if (!((vs[i][j][0])==(vs[0][0][0])||Math.abs((vs[i][j][0])-(vs[0][0][0]))<=tol)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1798 +"): " +"(" + "vs[i][j][0]" + ")==(" + "vs[0][0][0]" + ")+-" + "tol" + "" +" ("+(vs[i][j][0])+" vs. "+(vs[0][0][0])+" +- "+(tol)+")" +" (error = "+((vs[i][j][0])-(vs[0][0][0]))+")" ); } while (false);
                    if (i == 1)
                    {
                      do { if (!((ps[i][j][1])==(0.)||Math.abs((ps[i][j][1])-(0.))<=tol)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1801 +"): " +"(" + "ps[i][j][1]" + ")==(" + "0." + ")+-" + "tol" + "" +" ("+(ps[i][j][1])+" vs. "+(0.)+" +- "+(tol)+")" +" (error = "+((ps[i][j][1])-(0.))+")" ); } while (false);
                      do { if (!((vs[i][j][1])==(0.)||Math.abs((vs[i][j][1])-(0.))<=tol)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1802 +"): " +"(" + "vs[i][j][1]" + ")==(" + "0." + ")+-" + "tol" + "" +" ("+(vs[i][j][1])+" vs. "+(0.)+" +- "+(tol)+")" +" (error = "+((vs[i][j][1])-(0.))+")" ); } while (false);
                      if (false) // XXX not ready for this yet, but eventually... the slop can actually be useful since it tests tolerance for it, but should be systematic about it
                      {
                          do { if (!((ps[i][j][1])==(0.))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1805 +"): (" + "ps[i][j][1]" + ")" + "==" + "(" + "0." + ") ("+(ps[i][j][1])+" vs. "+(0.)+")"); } while (false);
                          do { if (!((vs[i][j][1])==(0.))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1806 +"): (" + "vs[i][j][1]" + ")" + "==" + "(" + "0." + ") ("+(vs[i][j][1])+" vs. "+(0.)+")"); } while (false);
                      }
                    }
                }
                ...
                if (!someoneIsMoving)
                    break;
                if (nItersDone >= maxIters)
                    break;

                for (int i = 0; i < 2; ++i)
                for (int j = 0; j < strategies.length; ++j)
                {
                    System.out.println("                    evolving particles["+i+"]["+j+"]...");
                    particles[i][j].evolve(referenceTripDistance,referenceTripTime, dt, distanceBasedBounce);
                }
        ...
        if (true)
        {
            // XXX wait, isn't this just trySomeEvolves?? yeah I think so...
            // A typical trip
            double p0[] = {1.,0.};
            double v0[] = {0.,1.};
            ...
            int strategies[] = {
                ...
                STRATEGY_TWO_PIECE_QUADRATIC,
                STRATEGY_TRY_TO_DO_IT_RIGHT,
                //STRATEGY_TRY_TO_DO_IT_RIGHT_WITH_SEPARABLE_BAILOUT, // no need to test this here
            };
            SmoothlyVaryingViewingParameter particles[] = new SmoothlyVaryingViewingParameter[strategies.length];
            for (int iStrategy = 0; iStrategy < strategies.length; ++iStrategy)
            {
                particles[iStrategy] = new SmoothlyVaryingViewingParameter(new double[2]);
                particles[iStrategy].setPosition(p0);
                particles[iStrategy].setVelocity(v0);
                particles[iStrategy].setTargetPosition(targetPosition);
                particles[iStrategy].strategy = strategies[iStrategy];
            }

            // [iStrategy][start][end]
            double ps[][] = new double[strategies.length][];
            while (true) // XXX huh? what was I doing here?
            {
                for (int iStrategy = 0; iStrategy < strategies.length; ++iStrategy)
                {
                    SmoothlyVaryingViewingParameter particleCopy = new SmoothlyVaryingViewingParameter(particles[iStrategy]);
                    for (int i = 0; particleCopy.isMoving(); ++i)
                    {
                        particleCopy.evolve(referenceTripDistance,referenceTripTime, i*dt, distanceBasedBounce);
                    }
                }
                break; // XXX huh? added this to prevent infinite loop... need to revisit what the hell was I doing
            }
        }

        if (false) // it works but takes a long time...
        {
            //
            // Test the double-quadratic behavior.
            // XXX or whatever the default is? I'm a bit confused.  maybe switch and switch back
            //
            ...
            x2.strategy = STRATEGY_TWO_PIECE_QUADRATIC;
            SmoothlyVaryingViewingParameter x3 = new SmoothlyVaryingViewingParameter(new double[2]);
            x3.strategy = STRATEGY_TWO_PIECE_QUADRATIC;
            double distanceBasedBounce = 0.; // XXX should write some tests with this varying
            double tol = 1e-5;
            ...
            for (int iReferenceTripDistance = 0; (iReferenceTripDistance) < (referenceTripDistances.length); ++iReferenceTripDistance)
            for (int iReferenceTripTime = 0; (iReferenceTripTime) < (referenceTripTimes.length); ++iReferenceTripTime)
            for (int iVStartX = 0; (iVStartX) < (vStartXs.length); ++iVStartX)
            for (int iVStartY = 0; (iVStartY) < (vStartYs.length); ++iVStartY)
            for (int iPStart = 0; (iPStart) < (pStarts.length); ++iPStart)
            for (int iVKneeY = 0; (iVKneeY) < (vKneeYs.length); ++iVKneeY)
            for (int iVKneeX = 0; (iVKneeX) < (vKneeXs.length); ++iVKneeX)
            for (int iIncr = 0; (iIncr) < (nIncrss.length); ++iIncr)
            {
                System.out.println("   ,==============================================");
                ...
                {System.out.print("        "); System.out.println("referenceTripDistance" + " = " + (referenceTripDistance));};
                {System.out.print("        "); System.out.println("referenceTripTime" + " = " + (referenceTripTime));};
                System.out.println("        " + "v" + " = " + Arrays.toStringCompact(v));
                System.out.println("        " + "p" + " = " + Arrays.toStringCompact(p));
                System.out.println("        " + "vKnee" + " = " + Arrays.toStringCompact(vKnee));
                {System.out.print("        "); System.out.println("nIncrs" + " = " + (nIncrs));};
                {System.out.print("        "); System.out.println("dt" + " = " + (dt));};
                ...
                //
                double a = 4*referenceTripDistance / ((referenceTripTime)*(referenceTripTime));
                ...
                {System.out.print("        "); System.out.println("expectedBeforeKneeTime" + " = " + (expectedBeforeKneeTime));};
                {System.out.print("        "); System.out.println("expectedAfterKneeTime" + " = " + (expectedAfterKneeTime));};
                {System.out.print("        "); System.out.println("expectedTotalTime" + " = " + (expectedTotalTime));};
                ...
                System.out.println("        " + "expectedTotalPositionChange" + " = " + Arrays.toStringCompact(expectedTotalPositionChange));
                double pTarget[] = VecMath.vpv(p, expectedTotalPositionChange);
                System.out.println("        " + "pTarget" + " = " + Arrays.toStringCompact(pTarget));
                ...
                if (true)
                {
                    ...
                    x2.get(scratch);
                    do { if (!(VecMath.equalsExactly(scratch, p))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1966 +"): " + "VecMath.equalsExactly(scratch, p)" + ""); } while (false);
                    x2.getVelocity(scratch);
                    do { if (!(VecMath.equalsExactly(scratch, VecMath.zerovec(p.length)))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1968 +"): " + "VecMath.equalsExactly(scratch, VecMath.zerovec(p.length))" + ""); } while (false);
                    do { if (!(!x2.isMoving())) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1969 +"): " + "!x2.isMoving()" + ""); } while (false);

                    x2.setTargetPosition(pTarget);
                    x2.get(scratch);
                    do { if (!(VecMath.equalsExactly(scratch, p))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1973 +"): " + "VecMath.equalsExactly(scratch, p)" + ""); } while (false);
                    x2.getVelocity(scratch);
                    do { if (!(VecMath.equalsExactly(scratch, VecMath.zerovec(p.length)))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1975 +"): " + "VecMath.equalsExactly(scratch, VecMath.zerovec(p.length))" + ""); } while (false);
                    do { if (!((x2.isMoving())==((VecMath.distsqrd(p,pTarget) != 0.)))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1976 +"): (" + "x2.isMoving()" + ")" + "==" + "(" + "(VecMath.distsqrd(p,pTarget) != 0.)" + ") ("+(x2.isMoving())+" vs. "+((VecMath.distsqrd(p,pTarget) != 0.))+")"); } while (false);

                    x2.setVelocity(v);
                    x2.get(scratch);
                    do { if (!(VecMath.equalsExactly(scratch, p))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1980 +"): " + "VecMath.equalsExactly(scratch, p)" + ""); } while (false);
                    x2.getVelocity(scratch);
                    do { if (!(VecMath.equalsExactly(scratch, v))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1982 +"): " + "VecMath.equalsExactly(scratch, v)" + ""); } while (false);
                    do { if (!((x2.isMoving())==((!VecMath.equalsExactly(p,pTarget) || VecMath.normsqrd(v) != 0.)))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+1983 +"): (" + "x2.isMoving()" + ")" + "==" + "(" + "(!VecMath.equalsExactly(p,pTarget) || VecMath.normsqrd(v) != 0.)" + ") ("+(x2.isMoving())+" vs. "+((!VecMath.equalsExactly(p,pTarget) || VecMath.normsqrd(v) != 0.))+")"); } while (false)
                                                                        ;
                    ...
                    for (int i = 0; i < nIncrs + 5.; ++i)
                    {
                        ...
                        {System.out.print("        "); System.out.println("t" + " = " + (t));};
                        {System.out.print("        "); System.out.println("t+dt" + " = " + (t+dt));};
                        x2.get(p0);
                        x2.getVelocity(v0);
                        x2.evolve(referenceTripDistance,referenceTripTime, dt, distanceBasedBounce);
                        t += dt;
                        x2.get(p1);
                        x2.getVelocity(v1);

                        System.out.println("        " + "p0" + " = " + Arrays.toStringCompact(p0));
                        System.out.println("        " + "v0" + " = " + Arrays.toStringCompact(v0));
                        System.out.println("        " + "p1" + " = " + Arrays.toStringCompact(p1));
                        System.out.println("        " + "v1" + " = " + Arrays.toStringCompact(v1));

                        if ((((expectedTotalTime)-(t)) > (tol)))
                            do { if (!(x2.isMoving())) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2010 +"): " + "x2.isMoving()" + ""); } while (false);
                        else if ((((t)-(expectedTotalTime)) > (tol)))
                            do { if (!(!x2.isMoving())) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2012 +"): " + "!x2.isMoving()" + ""); } while (false);
                        else
                        {
                            // should be pretty damn close to stopped
                            do { if (!(VecMath.norm(v1) <= 1e-6)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2016 +"): " + "VecMath.norm(v1) <= 1e-6" + ""); } while (false);
                            do { if (!(VecMath.dist(p1, pTarget) <= 1e-6)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2017 +"): " + "VecMath.dist(p1, pTarget) <= 1e-6" + ""); } while (false);
                        }

                        {
                            ...
                            allAtOnce.setTargetPosition(pTarget);
                            allAtOnce.evolve(referenceTripDistance,referenceTripTime, t, distanceBasedBounce);
                            double p1AllAtOnce[] = new double[2];
                            double v1AllAtOnce[] = new double[2];
                            allAtOnce.get(p1AllAtOnce);
                            allAtOnce.getVelocity(v1AllAtOnce);
                            System.out.println("        " + "p1" + " = " + Arrays.toStringCompact(p1));
                            System.out.println("        " + "p1AllAtOnce" + " = " + Arrays.toStringCompact(p1AllAtOnce));
                            System.out.println("        " + "v1" + " = " + Arrays.toStringCompact(v1));
                            System.out.println("        " + "v1AllAtOnce" + " = " + Arrays.toStringCompact(v1AllAtOnce));

                            do { if (!(VecMath.dist(p1, p1AllAtOnce) <= 1e-6)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2036 +"): " + "VecMath.dist(p1, p1AllAtOnce) <= 1e-6" + ""); } while (false);
                            do { if (!(VecMath.dist(v1, v1AllAtOnce) <= 1e-6)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2037 +"): " + "VecMath.dist(v1, v1AllAtOnce) <= 1e-6" + ""); } while (false);
                        }
                ...
                if (false)
                {
                    do { if (!(false)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2050 +"): " + "false" + ""); } while (false); // XXX DO ME
                }
            }
...

        {
            ...
            x.setTargetPosition(1.);
            {System.out.print("        "); System.out.println("x.get()" + " = " + (x.get()));};
            {System.out.print("        "); System.out.println("x.getTargetPosition()" + " = " + (x.getTargetPosition()));};
            {System.out.print("        "); System.out.println("x.getVelocity()" + " = " + (x.getVelocity()));};
            x.evolve(1, 1, .1, 0);
            {System.out.print("        "); System.out.println("x.get()" + " = " + (x.get()));};
            //System.exit(0);
        }
        ...
        if (true)
        {
            double distanceBasedBounce = 0.; // XXX should write some tests with this
            {
                SmoothlyVaryingViewingParameter x = new SmoothlyVaryingViewingParameter(0.);
                x.setTargetPosition(1.);
                ...
                for (int i = 0; i < 15; ++i)
                {
                    ...
                    System.out.println("    Before evolve: "+
                        com.donhatchsw.compat.Format.sprintf("t = %20.17g p = %20.17g  v = %20.17g",
                                            t, p0, v0));
                    x.evolve(1,1, dt, distanceBasedBounce);
                    t += dt;
                    double p1 = x.get();
                    ...
                    System.out.println("     After evolve: "+
                        com.donhatchsw.compat.Format.sprintf("t = %20.17g p = %20.17g  v = %20.17g",
                                            t, p1, v1));
                    if ((((1.)-(t)) > (tol)))
                        do { if (!((((1.)-(x.get())) > (tol)))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2108 +"): " + "LT(x.get(), 1., tol)" + ""); } while (false);
                    else
                        do { if (!((x.get())==(1.))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2110 +"): (" + "x.get()" + ")" + "==" + "(" + "1." + ") ("+(x.get())+" vs. "+(1.)+")"); } while (false);
                    System.out.println();
                }
                do { if (!((x.get())==(1.))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2113 +"): (" + "x.get()" + ")" + "==" + "(" + "1." + ") ("+(x.get())+" vs. "+(1.)+")"); } while (false);
                do { if (!((x.getVelocity())==(0.))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2114 +"): (" + "x.getVelocity()" + ")" + "==" + "(" + "0." + ") ("+(x.getVelocity())+" vs. "+(0.)+")"); } while (false);
            }
            System.out.println("================================================");
        }
            ...
            for (int i = 0; i < 55; ++i)
            {
                ...
                System.out.println("     After evolve: "+
                    com.donhatchsw.compat.Format.sprintf("t = %20.17g p = %20.17g  v = %20.17g",
                                        t, p1, v1));
                if ((((t)-(2.5)) <= (tol)))
                    do { if (!(p1 > p0)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2149 +"): " + "p1 > p0" + ""); } while (false);
                else if ((((t)-(5.)) <= (tol)))
                    do { if (!(p1 < p0)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2151 +"): " + "p1 < p0" + ""); } while (false);
                else
                {
                    do { if (!((p0)==(1.))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2154 +"): (" + "p0" + ")" + "==" + "(" + "1." + ") ("+(p0)+" vs. "+(1.)+")"); } while (false);
                    do { if (!((p1)==(1.))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2155 +"): (" + "p1" + ")" + "==" + "(" + "1." + ") ("+(p1)+" vs. "+(1.)+")"); } while (false);
                }
                System.out.println();
            }
            do { if (!((x.get())==(1.))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2159 +"): (" + "x.get()" + ")" + "==" + "(" + "1." + ") ("+(x.get())+" vs. "+(1.)+")"); } while (false);
        }
...

        {
            ...
            for (int iCanonicalDistance = 0; (iCanonicalDistance) < (canonicalDistances.length); ++iCanonicalDistance)
            for (int iCanonicalTime = 0; (iCanonicalTime) < (canonicalTimes.length); ++iCanonicalTime)
            for (int iStart = 0; (iStart) < (starts.length); ++iStart)
            ...
            for (int iIncr = 0; (iIncr) < (nIncrss.length); ++iIncr)
            {
                ...
                {System.out.print("        "); System.out.println("canonicalDistance" + " = " + (canonicalDistance));};
                {System.out.print("        "); System.out.println("canonicalTime" + " = " + (canonicalTime));};
                {System.out.print("        "); System.out.println("start" + " = " + (start));};
                {System.out.print("        "); System.out.println("target" + " = " + (target));};
                {System.out.print("        "); System.out.println("nIncrs" + " = " + (nIncrs));};
                {System.out.print("        "); System.out.println("dt" + " = " + (dt));};
                ...
                {System.out.print("        "); System.out.println("x.get()" + " = " + (x.get()));};
                {System.out.print("        "); System.out.println("x.getVelocity()" + " = " + (x.getVelocity()));};
                do { if (!((x.get())==(start))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2202 +"): (" + "x.get()" + ")" + "==" + "(" + "start" + ") ("+(x.get())+" vs. "+(start)+")"); } while (false);
                do { if (!((x.getVelocity())==(0.))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2203 +"): (" + "x.getVelocity()" + ")" + "==" + "(" + "0." + ") ("+(x.getVelocity())+" vs. "+(0.)+")"); } while (false);
                do { if (!(!x.isMoving())) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2204 +"): " + "!x.isMoving()" + ""); } while (false);

                x.setTargetPosition(target);

                int overallDir = (((target)-(start)) > (tol)) ? 1 : (((start)-(target)) > (tol)) ? -1 : 0;
                double t = 0.;
                for (int i = 0; i < nIncrs + 5; ++i)
                {
                    double p0 = x.get();
                    ...
                    if (false)
                        System.out.println("    Before evolve: "+
                            com.donhatchsw.compat.Format.sprintf("t = %20.17g p = %20.17g  v = %20.17g",
                                                t, p0, v0));
                    x.evolve(canonicalDistance,canonicalTime, dt, distanceBasedBounce);
                    t += dt;
                    double p1 = x.get();
                    double v1 = x.getVelocity();
                    if (false)
                        System.out.println("     After evolve: "+
                            com.donhatchsw.compat.Format.sprintf("t = %20.17g p = %20.17g  v = %20.17g",
                                                t, p1, v1));
                    {System.out.print("        "); System.out.println("overallDir" + " = " + (overallDir));};
                    {System.out.print("        "); System.out.println("p0" + " = " + (p0));};
                    {System.out.print("        "); System.out.println("p1" + " = " + (p1));};
                    if (overallDir > 0)
                        do { if (!((((p0)-(p1)) <= (tol)))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2231 +"): " + "GEQ(p1,p0,tol)" + ""); } while (false);
                    else if (overallDir < 0)
                        do { if (!((((p1)-(p0)) <= (tol)))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2233 +"): " + "LEQ(p1,p0,tol)" + ""); } while (false);
                    else
                        do { if (!(((((p1)-(p0)) <= (tol)) && (((p0)-(p1)) <= (tol))))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2235 +"): " + "EQ(p1,p0,tol)" + ""); } while (false);
        ...
        System.out.println("    out confidenceTests");
            ...
            private static double[][] List(double a[], double b[], double c[], double d[]) {return new double[][]{a,b,c,d};}
            private static final double Power(double x, int y) {do { if (!((y)==(2))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2253 +"): (" + "y" + ")" + "==" + "(" + "2" + ") ("+(y)+" vs. "+(2)+")"); } while (false); return x*x;}
            private static final double Power3(double x, int y) {do { if (!((y)==(3))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2254 +"): (" + "y" + ")" + "==" + "(" + "3" + ") ("+(y)+" vs. "+(3)+")"); } while (false); return x*x*x;}
                ...
                if (twoQuadraticsMode)
                {
                    ...
                    if (t < switchTime)
                    {
                        ...
                        VecMath.vpsxv(answer,
                                      answer,
                                      ((t)*(t))/2, VecMath.normalize(VecMath.lerp(A0,A1, switchTime-1.)));
                        return answer;
                    }
                    else
                    {
                        ...
                        VecMath.vpsxv(answer,
                                      answer,
                                      ((switchTime)*(switchTime))/2, VecMath.normalize(VecMath.lerp(A0,A1, switchTime-1.)));
                        VecMath.vpsxv(answer,
                                      answer,
                                      t-switchTime, switchV);
                        VecMath.vpsxv(answer,
                                      answer,
                                      ((t-switchTime)*(t-switchTime))/2, VecMath.normalize(VecMath.lerp(A0,A1, switchTime+1.)));
                        return answer;
                    }
                }
                else
...
Print["J = ",CForm[J]]
      ...
      {-((-x0 + xt1)/
              ...
              Sqrt(Power(-x0 + xt1, 2) + Power(-y0 + yt1, 2))},
                ...
                {System.out.print("        "); System.out.println("answer.length" + " = " + (answer.length));};
                {System.out.print("        "); System.out.println("answer[0].length" + " = " + (answer[0].length));};
                do { if (!((answer.length)==(4))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2587 +"): (" + "answer.length" + ")" + "==" + "(" + "4" + ") ("+(answer.length)+" vs. "+(4)+")"); } while (false);
                do { if (!((answer[0].length)==(4))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+2588 +"): (" + "answer[0].length" + ")" + "==" + "(" + "4" + ") ("+(answer[0].length)+" vs. "+(4)+")"); } while (false);
                return answer;
...
(-(((x0 - x1)*((-1 + t)*Power(x0, 2) + x0*(x1 - 2*t*x1) +
              ...
              1.5)*(-Power(x0, 2) + t*Power(x0, 2) + x0*x1 - 2*t*x0*x1 +
                ...
                do { if (!((answer.length)==(5))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+4963 +"): (" + "answer.length" + ")" + "==" + "(" + "5" + ") ("+(answer.length)+" vs. "+(5)+")"); } while (false);
                return answer;
...
(-(((y0 - y1)*((-1 + t)*Power(x0, 2) +
              ...
              1.5)*(-Power(x0, 2) + t*Power(x0, 2) + x0*x1 - 2*t*x0*x1 +
                ...
                do { if (!((answer.length)==(5))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+7339 +"): (" + "answer.length" + ")" + "==" + "(" + "5" + ") ("+(answer.length)+" vs. "+(5)+")"); } while (false);
                return answer;
    ...
    (((2*(x0 - x1)*
                ...
                do { if (!((answer.length)==(5))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+9709 +"): (" + "answer.length" + ")" + "==" + "(" + "5" + ") ("+(answer.length)+" vs. "+(5)+")"); } while (false);
                return answer;
 ...
 (((2*Sqrt(Power(x0, 2) + Power(y0, 2))*(y0 - y1))/(Power(x0, 2) -
                ...
                do { if (!((answer.length)==(5))) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+12076 +"): (" + "answer.length" + ")" + "==" + "(" + "5" + ") ("+(answer.length)+" vs. "+(5)+")"); } while (false);
                return answer;
                ...
                extends MyFun
            {
                ...
                @Override public void J(double x[],
                                       double fx[], // unused
                                       double answer[][])
                {
                    double A0[] = {x[0],x[1]};
                    ...
                    double vt1[] = calcVelocity(v0, A0, A1, t1, twoQuadraticsMode);
                    double pt1[] = calcPosition(p0,v0, A0, A1, t1, twoQuadraticsMode);
                    VecMath.copymat(answer, calcVelocityAndPositionJacobianWithRespectToA0At1(p0,v0,A0,At1,sign_t1, twoQuadraticsMode)); // XXX memory allocation
                    do { if (!(false)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+12130 +"): " + "false" + ""); } while (false); // coverage  XXX currently never called
                }
                                                 ...
                                                 double At1InitialGuess[])
            {
                ...
                //NewtonSolver.Fun fun = new MyFunTryingToBeSmart(p0,v0,sign_t1,twoQuadraticsMode); // bombs :-(   XXX does that mean the thousands of lines of code are for naught??? wtf? look into this
                ...
                NewtonSolver.solve(x, y, fun, minIterations, maxIterations, adaptiveFlag);
                double A0[] = {x[0],x[1]};
                double At1[] = {x[2],x[3]};
    ...
    // DOING THIS:
    //     Evolve from now by dt, 2*dt, 3*dt, ... until calm,
    //     then evolve by each of those to each other, and make sure we get same result.
    // COULD DO THIS INSTEAD but more prone to roundoff:
    //     first do it by dt increments
    private static void trySomeEvolves(double[] initialPosition,
                                       double[] targetPosition,
                                       double[] initialVelocity,
                                       double dt,
                                       double mostRecentNewtonPathHolder[][][][],
                                       int interestingIndex)
    {
        System.out.println("    in trySomeEvolves");
        System.out.println("        " + "initialPosition" + " = " + VecMath.toString(initialPosition));
        System.out.println("        " + "targetPosition" + " = " + VecMath.toString(targetPosition));
        System.out.println("        " + "initialVelocity" + " = " + VecMath.toString(initialVelocity));
        // to get A=1...
        double referenceTripDistance = .25;
        double referenceTripTime = 1.;
        double distanceBasedBounce = 0.; // probably not prepared to do anything with this

        com.donhatchsw.compat.ArrayList positionsArrayList = new com.donhatchsw.compat.ArrayList();
        com.donhatchsw.compat.ArrayList velocitiesArrayList = new com.donhatchsw.compat.ArrayList();

        for (int i = 0; ; i++)
        {
            SmoothlyVaryingViewingParameter foo = new SmoothlyVaryingViewingParameter(initialPosition);
            foo.strategy = STRATEGY_TRY_TO_DO_IT_RIGHT;
            foo.setVelocity(initialVelocity);
            foo.setTargetPosition(targetPosition);
            double p[] = new double[initialPosition.length];
            double v[] = new double[initialPosition.length];
            foo.evolve(referenceTripDistance,
                       referenceTripTime,
                       i * dt,
                       distanceBasedBounce);
            foo.get(p);
            foo.getVelocity(v);
            System.out.println("        i = "+i+" -> p="+Arrays.toStringCompact(p)+" v="+Arrays.toStringCompact(v));
            if (Double.isNaN(VecMath.norm(v))
             || Double.isNaN(VecMath.norm(p))
             || Double.isInfinite(VecMath.norm(v))
             || Double.isInfinite(VecMath.norm(p)))
            {
                System.out.println("        (ouch.)");
                break;
            }
            positionsArrayList.add(p);
            velocitiesArrayList.add(v);

            if (!foo.isMoving())
                break;
        }
        int n = positionsArrayList.size();
        double positions[][] = (double[][])positionsArrayList.toArray(new double[n][]);
        double velocities[][] = (double[][])velocitiesArrayList.toArray(new double[n][]);
        System.out.println("        " + "positions" + " =\n" + Arrays.toStringNonCompact(positions, "        ", "        "));
        System.out.println("        " + "velocities" + " =\n" + Arrays.toStringNonCompact(velocities, "        ", "        "));

        for (int i = 0; i < n; ++i) // i=0 is redundant except for debugging
        {
            for (int j = i; j < n; ++j)
            {
                double pExpected[] = positions[j];
                double vExpected[] = velocities[j];

                SmoothlyVaryingViewingParameter foo = new SmoothlyVaryingViewingParameter(positions[i]);
                foo.strategy = STRATEGY_TRY_TO_DO_IT_RIGHT;
                foo.setVelocity(velocities[i]);
                foo.setTargetPosition(targetPosition);
                double pActual__[] = new double[initialPosition.length];
                double vActual__[] = new double[initialPosition.length];
                int verboseLevel = i==interestingIndex&&j==interestingIndex ? 3 : 0; // XXX YYY
                com.donhatchsw.compat.ArrayList newtonPathArrayList = i==interestingIndex&&j==interestingIndex ? new com.donhatchsw.compat.ArrayList() : null;
                foo.evolve(referenceTripDistance,
                           referenceTripTime,
                           (j-i) * dt,
                           distanceBasedBounce,
                           verboseLevel,
                           newtonPathArrayList);
                if (newtonPathArrayList != null)
                {
                    double newtonPath[][][] = (double[][][])newtonPathArrayList.toArray(new double[newtonPathArrayList.size()][][]);
                    System.out.println("        " + "newtonPath" + " = " + Arrays.toStringCompact(newtonPath));
                    {System.out.print("        "); System.out.println("newtonPath.length" + " = " + (newtonPath.length));}
                    mostRecentNewtonPathHolder[0] = newtonPath;
                }
                foo.get(pActual__);
                foo.getVelocity(vActual__);
                System.out.println("      "+i+"->"+j+" / "+n+" :");
                System.out.println("        " + "pExpected" + " = " + Arrays.toStringCompact(pExpected));
                System.out.println("        " + "pActual__" + " = " + Arrays.toStringCompact(pActual__));
                System.out.println("        " + "vExpected" + " = " + Arrays.toStringCompact(vExpected));
                System.out.println("        " + "vActual__" + " = " + Arrays.toStringCompact(vActual__));
                {System.out.print("        "); System.out.println("VecMath.dist(pExpected, pActual__)" + " = " + (VecMath.dist(pExpected, pActual__)));};
                {System.out.print("        "); System.out.println("VecMath.dist(vExpected, vActual__)" + " = " + (VecMath.dist(vExpected, vActual__)));};
                double tol = 1e-12;
                do { if (!(VecMath.dist(pExpected, pActual__) <= tol)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+12275 +"): " + "VecMath.dist(pExpected, pActual__) <= tol" + ""); } while (false);
                do { if (!(VecMath.dist(vExpected, vActual__) <= tol)) throw new Error("CHECK failed at "+"com/donhatchsw/util/SmoothlyVaryingViewingParameter.prejava"+"("+12276 +"): " + "VecMath.dist(vExpected, vActual__) <= tol" + ""); } while (false);

                if (i == interestingIndex && j == interestingIndex+1) break;
            }
            if (i == interestingIndex) break;
        }

        System.out.println("    out trySomeEvolves");
    } // trySomeEvolves
    ...
    private static void interactiveDemo2()
    {
        ...
        java.awt.Canvas canvas = new java.awt.Canvas() {
            ...
            class Model
            {
                ...
                // nice case to debug
                public double A0[] = {.17,-1.6};
                public double A1[] = {.17,1.6}; // will be adjusted to be unit distance away XXX
                public double t1 = VecMath.dist(A0,A1); // so A1 actually turns into At1
                ...
                public double mostRecentNewtonPath[][][] = null;
                public int interestingIndex = -1;
                //public int interestingIndex = 6;
                //public int interestingIndex = 24;
            ...
            class View
            {
                ...
                boolean showAccelerationsFlag = true; // XXX currently no way to change
                boolean showVelocitiesFlag = true; // XXX currently no way to change
                boolean showPositionsFlag = true; // XXX currently no way to change
                boolean showAccelerationLines = false;
                boolean showVelocityHandles = false;
                boolean finalVelocityHandleIsNegative = false;
                double intensityMultiplier = 0.; // gets auto scaled based on where user is clicking
                double scale = .24;
                double focus[] = {0.,0.};
            }
            class Controller
            ...
            @Override public void paint(java.awt.Graphics frontBufferGraphics)
            {
                ...
                // We actually change the graphics scale depending on the acceleration magnitude,
                // and change the size of the unit circle to compensate!  Argh!
                // So are we actually not using accelerationMagnitude in the computation at all?  Argh!!
                com.donhatchsw.awt.MyGraphics mg = new com.donhatchsw.awt.MyGraphics(g, this.getSize(),
                                              view.focus[0] + -1.*w/minSize/(view.scale*model.accelerationMagnitude),
                                              view.focus[0] + 1.*w/minSize/(view.scale*model.accelerationMagnitude),
                                              view.focus[1] + -1.*h/minSize/(view.scale*model.accelerationMagnitude),
                                              view.focus[1] + 1.*h/minSize/(view.scale*model.accelerationMagnitude));
                ...
                if (Double.isNaN(t1))
                {
                    System.out.println("WARNING: t1 is NaN, wtf?"); // TODO: track down source of badness
                    return;
                }
                ...
                if (false)
                {
                    System.out.println("        " + "A0" + " = " + VecMath.toString(A0));
                    System.out.println("        " + "At1" + " = " + VecMath.toString(At1));
                    System.out.println("        " + "vt1" + " = " + VecMath.toString(vt1));
                    System.out.println("        " + "pt1" + " = " + VecMath.toString(pt1));
                    ...
                    System.out.println("        " + "A0" + " = " + VecMath.toString(A0));
                    System.out.println("        " + "At1" + " = " + VecMath.toString(At1));
                    System.out.println("        " + "solvedA0" + " = " + VecMath.toString(solvedA0));
                    System.out.println("        " + "solvedAt1" + " = " + VecMath.toString(solvedAt1));
                }
                 ...
                 || controller.mostRecentThingBeingDragged == Model.PICK_At1)
                {
                    ...
                    for (int iX = 0; (iX) < (nGridSegs+1); ++iX)
                    for (int iY = 0; (iY) < (nGridSegs+1); ++iY)
                    {
                        double A0_[], At1_[];
                        if (controller.mostRecentThingBeingDragged == Model.PICK_A0)
                        {
                    ...
                    else
                    {
                        ...
                        for (int iY = 0; (iY) < (nGridSegs+1); ++iY)
                        {
                            ...
                            if (view.showAccelerationsFlag)
                            {
                                mg.setColor(thisRed);
                                mg.drawLine(At1_0[0],At1_0[1],
                                            At1_1[0], At1_1[1]);
                            }
                            if (view.showVelocitiesFlag)
                            {
                                mg.setColor(thisGreen);
                ...
                {
                    int nSubdivs = 10;
                    double max = sign_t1*t1 * nSubdivs;
                    if (max > 10000)
                    {
                        System.out.println("WARNING: clamping max from "+max+" to 10000");
                        max = 10000;
                    }
                    for (int i = 0; i <= max; ++i)
                    {
                        ...
                        if (view.showPositionsFlag)
                        {
                            ...
                            //System.out.print("["+i+"/("+t1+"*"+nSubdivs+")]");
                            //System.out.flush();
                ...
                if (view.showVelocitiesFlag
                 && view.showPositionsFlag
                 && VecMath.equalsExactly(model.p0, model.v0))
                {
                ...
                if (view.showAccelerationsFlag)
                {
                    ...
                    if (twoQuadraticsMode)
                    {
                        ...
                        if ((((switchTime)-(0)) > (1e-3))
                         && (((t1)-(switchTime)) > (1e-3)))
                        {
                            mg.drawPoint(Aswitch[0], Aswitch[1], 5);
                            if (!((((switchTime)-(1)) <= (1e-3)) && (((1)-(switchTime)) <= (1e-3))))
...

                {
                    ...
                    g.drawString("h for more detailed help to terminal window", 20,y-=20);
                ...
                {
                    // Most recent debug newton path, if there is one
                    if (model.mostRecentNewtonPath != null)
                    {
                        System.out.println("DRAWING MOST RECENT NEWTON PATH");

                        java.awt.Color reddish;
                        java.awt.Color greenish;
                        java.awt.Color yellowish;
                        {
                            float reddishRGB[] = new float[3];
                            float greenishRGB[] = new float[3];
                            float yellowishRGB[] = new float[3];
                            HSVtoRGB(0.f,.4f,1.f,reddishRGB);
                            HSVtoRGB(1.f/3.f,.4f,1.f,greenishRGB);
                            HSVtoRGB(1.f/6.f,.4f,1.f,yellowishRGB);
                            reddish = new java.awt.Color(reddishRGB[0], reddishRGB[1], reddishRGB[2]);
                            greenish = new java.awt.Color(greenishRGB[0], greenishRGB[1], greenishRGB[2]);
                            yellowish = new java.awt.Color(yellowishRGB[0], yellowishRGB[1], yellowishRGB[2]);
                        }

                        double xPrev[] = null;
                        double yPrev[] = null;
                        double midPrev[] = null;
                        for (int i = 0; i < model.mostRecentNewtonPath.length; i++)
                        {
                            double x[] = model.mostRecentNewtonPath[i][0];
                            double y[] = model.mostRecentNewtonPath[i][1];
                            if (x != null)
                            {
                                if (Double.isNaN(x[0])
                                 || Double.isNaN(x[1])
                                 || Double.isNaN(y[0])
                                 || Double.isNaN(y[1])
                                 || Double.isInfinite(x[0])
                                 || Double.isInfinite(x[1])
                                 || Double.isInfinite(y[0])
                                 || Double.isInfinite(y[1]))
                                    continue;
                                double xMid[] = VecMath.lerp(x, y, 1./4.);
                                double mid[] = VecMath.lerp(x, y, .5);
                                double yMid[] = VecMath.lerp(x, y, 3./4.);

                                mg.setColor(reddish);
                                mg.drawLine(x[0],x[1], xMid[0],xMid[1]);
                                if (xPrev != null)
                                    mg.drawLine(xPrev[0],xPrev[1], x[0],x[1]);

                                mg.setColor(yellowish);
                                mg.drawLine(xMid[0],xMid[1], yMid[0],yMid[1]);
                                if (midPrev != null)
                                    mg.drawLine(midPrev[0],midPrev[1], mid[0],mid[1]);

                                mg.setColor(greenish);
                                mg.drawLine(yMid[0],yMid[1], y[0],y[1]);
                                if (yPrev != null)
                                    mg.drawLine(yPrev[0],yPrev[1], y[0],y[1]);
                        ...
                        for (int i = 0; i < model.mostRecentNewtonPath.length; i++)
                        {
                            ...
                            if (x == null)
                            {
                                ...
                                mg.drawHollowCirclePoint(y[0],y[1],
                                                         1); // pointSize
                                mg.drawHollowCirclePoint(y[0],y[1],
                                                         5); // pointSize
                                mg.drawHollowCirclePoint(y[0],y[1],
                        ...
                        System.out.println("DREW MOST RECENT NEWTON PATH");
                    }
                }
            ...
            } // paint
            {
                addKeyListener(new java.awt.event.KeyAdapter() {
                    public void keyPressed(java.awt.event.KeyEvent e)
                    {
                        ...
                        else if (c == '-' || c == '+' || c == '=')
                        {
                            ...
                            if (altIsDown)
                            {
                                model.interestingIndex += (c=='-' ? -1 : 1);
                                {System.out.print("        "); System.out.println("model.interestingIndex" + " = " + (model.interestingIndex));};

                                // Ridiculous hack: inject an 'e' event, to try some evolves
                                java.awt.event.KeyEvent syntheticKeyEvent = new java.awt.event.KeyEvent(
                                    e.getComponent(),
                                    e.getID(),
                                    e.getWhen(),
                            ...
                            else if (controlIsDown)
                            {
                                ...
                                {System.out.print("        "); System.out.println("model.accelerationMagnitude" + " = " + (model.accelerationMagnitude));};
                        ...
                        else if (c == 'q' || c == 'Q')
                        {
                            ...
                            {System.out.print("        "); System.out.println("model.twoQuadraticsMode" + " = " + (model.twoQuadraticsMode));};
                        ...
                        {
                            System.out.println(""+"==================================");
                            System.out.println(""+"        Trying some evolves");

                            // Messing around here... do some evolves.
                            // Can only do this if v0 is at 0.
                            System.out.println("        " + "model.v0" + " = " + Arrays.toStringCompact(model.v0));
                            if (VecMath.normsqrd(model.v0) == 0.)
                            {
                                double vt1[] = calcVelocity(model.v0,model.A0,model.A1,model.t1,model.twoQuadraticsMode);
                                double pt1[] = calcPosition(model.p0,model.v0,model.A0,model.A1,model.t1,model.twoQuadraticsMode);
                                if (model.t1 > 0.)
                                    VecMath.vxs(vt1, vt1, -1.);
                                double initialPosition[] = {pt1[0], pt1[1], 0};
                                double targetPosition[] = {model.p0[0], model.p0[1], 0};
                                double initialVelocity[] = {vt1[0], vt1[1], 0};
                                double dt = .1;
                                double mostRecentNewtonPathHolder[][][][] = {model.mostRecentNewtonPath};
                                trySomeEvolves(initialPosition, targetPosition, initialVelocity, dt, mostRecentNewtonPathHolder, model.interestingIndex);
                                model.mostRecentNewtonPath = mostRecentNewtonPathHolder[0];
                                repaint();
                            }
                            else
                            {
                                System.out.println("    can't do it; v(0) isn't at 0");
                            }
                            System.out.println(""+"==================================");
                        }
                        else if (c == 'c')
                        {
                            // try to put the target at the center of the screen.
                            if (model.mostRecentNewtonPath != null)
                            {
                                for (int i = 0; i < model.mostRecentNewtonPath.length; ++i)
                                {
                                    if (model.mostRecentNewtonPath[i][0] == null)
                                    {
                                        double target[] = model.mostRecentNewtonPath[i][1];
                                        view.focus[0] = target[0];
                                        view.focus[1] = target[1];
                                        repaint();
                                        break;
                                    }
                                }
                        ...
                        else if (c == 'h')
                        {
                            ...
                            System.out.println("  ctrl-(-/+/=) - decrease/increase acceleration magnitude");
                            System.out.println("             r - reset");
                            System.out.println("             a - toggle acceleration segments");
                            System.out.println("             v - toggle velocity handles");
                            System.out.println("             q - toggle two-quadratics mode (non-optimal but simpler to calculate)");
                            System.out.println("             h - show this help message");
                            System.out.println("           Esc - quit");
                            System.out.println("    Advanced:");
                            // toggle between t1 positive and negative,
                            // adjusting everything so no pop
                            System.out.println("             a - toggle acceleration lines");
                            System.out.println("        ctrl-v - toggle direction of one of the velocity handles");
                            System.out.println("             t - toggle between t1 positive and negative, adjusting everything so position trajectory is same (XXX needs work)");
                            System.out.println("             e - do some evolves");
                            System.out.println("   alt-(-/+/=) - decrease/increase index of interest in newton path, and do some evolves");
                            System.out.println("             c - change focus to the target of the index of interest in the most recent newton path so you can zoom in and out around it");
                        }
                        else
                        {
                ...
                addMouseListener(new java.awt.event.MouseListener() {
                    ...
                    public void mousePressed(java.awt.event.MouseEvent e)
                    {
                        ...
                        if (eventVerbose >= 1) {System.out.print("        "); System.out.println("controller.indexOfThingBeingDragged" + " = " + (controller.indexOfThingBeingDragged));};
                    ...
                    public void mouseMoved(java.awt.event.MouseEvent e)
                    {
                        ...
                        if (mostRecentGraphics != null)
                        {
                             ...
                             || controller.mostRecentThingBeingDragged == Model.PICK_At1)
                            {
                                ...
                                {
                                    controller.mostRecentThingBeingDragged = Model.PICK_At1;
                                    view.intensityMultiplier = (((frac-.5)/(.75-.5))<=(0)?(0):((frac-.5)/(.75-.5))>=(1)?(1):((frac-.5)/(.75-.5)));
                                }
                                else
                                {
                                    controller.mostRecentThingBeingDragged = Model.PICK_A0;
                                    view.intensityMultiplier = (((frac-.5)/(.25-.5))<=(0)?(0):((frac-.5)/(.25-.5))>=(1)?(1):((frac-.5)/(.25-.5)));
                                }
                    ...
                    public void mouseDragged(java.awt.event.MouseEvent e)
                    {
                        ...
                        else if (controller.indexOfThingBeingDragged == Model.PICK_velocityKnee)
                        {
                              ...
                              * twiceTriangleArea(v0,vt1,newKnee,0,1) < 0)
                            {
                                ...
                                model.A0 = At1;
                                {System.out.print("        "); System.out.println("\"YEAH!\"" + " = " + ("YEAH!"));};
                            }
                        }
                        else
                        {
        ...
        java.awt.Canvas canvas = new java.awt.Canvas() {
            ...
            @Override public void paint(java.awt.Graphics frontBufferGraphics)
            {
                ...
                mg.drawPoint(-1+model.referenceTripTime,
                             -1+model.referenceTripDistance,
                             3);
                double distanceBasedBounce = Math.max(model.unclampedBounce, 0);
                // And where we think the first bounce will be
                mg.drawPoint(-1+model.referenceTripTime*(1+Math.sqrt(distanceBasedBounce)),
                             -1+model.referenceTripDistance*(1-distanceBasedBounce),
                             3);
                // And the second one
                if (false) // nah, can't drag it so it's confusing
                    mg.drawPoint(-1+model.referenceTripTime*(1+Math.sqrt(distanceBasedBounce+distanceBasedBounce)),
                                 -1+model.referenceTripDistance*(1-distanceBasedBounce+distanceBasedBounce*distanceBasedBounce),
                                 3);
                ...
                // Draw the reference trip
                {
                    ...
                    //                  (1 - b + b^2 - ...)
                    param.setTargetPosition(-1. + model.referenceTripDistance / (1.+distanceBasedBounce));
            ...
            }
            {
                addMouseListener(new java.awt.event.MouseListener() {
                    ...
                    public void mousePressed(java.awt.event.MouseEvent e)
                    {
                        ...
                        if (eventVerbose >= 1) {System.out.print("        "); System.out.println("indexOfThingBeingDragged" + " = " + (indexOfThingBeingDragged));};
    ...
    private static void interactiveDemo3()
    {
        ...
        java.awt.Canvas canvas = new java.awt.Canvas() {
            ...
            class Model
            {
                ...
                public boolean dumpGoodAndBadFlag = true;
                public String goodAndBadOutFileName = "GOOD_AND_BAD_OUT";
                public java.io.PrintWriter goodAndBadOut = null;
                public long evolveTimesNanos[] = new long[10*1000];
                public long planTimesNanos[] = new long[10*1000];
                ...
                {
                    if (this.dumpGoodAndBadFlag && this.goodAndBadOut == null)
                    {
                        try {
                            this.goodAndBadOut = new java.io.PrintWriter(new java.io.FileWriter(this.goodAndBadOutFileName));
                        }
                        catch (java.io.IOException e)
                        {
                            System.err.println("Error trying to open file "+this.goodAndBadOutFileName+" for writing: "+e);
                            this.dumpGoodAndBadFlag = false; // so we don't keep trying
                        }
                    }
                    CatenaryRotated.goodAndBadOut = (this.dumpGoodAndBadFlag ? this.goodAndBadOut : null);
            ...
            class View
            {
                ...
                int nPlansToShow = 0;
                boolean showPlanAccelerationsFlag = false;
                boolean antialiasFlag = true;
                boolean showTimingsFlag = false;
                double timingsPixelsPerMs = 2.;
            ...
            @Override public void paint(java.awt.Graphics frontBufferGraphics)
            {
                 ...
                 || !size.equals(backBufferSize))
                {
                    ...
                    backBuffer = this.createImage(w, h);
                    backBufferSize = size;
                    // After window resize, set target to window center
                    for (int I = 0; (I) < (model.points.length); ++I)
                        model.points[I].setTargetPosition(new double[]{w*.5, h*.5});

                    if (false)
                    {
                        // to 0,0
                        System.out.println("    HACK for debugging: setting target to 0,0");
                        for (int I = 0; (I) < (model.points.length); ++I)
                        {
                            //model.points[I].setTargetPosition(new double[]{0., 0.});

                            double position[] = new double[2];
                            model.points[I].getTargetPosition(position);
                            position[1] += 100.;
                            model.points[I].setPosition(position);

                            //model.points[I].setVelocity(new double[]{-1., 0.});
                            model.points[I].setVelocity(new double[]{0., 0.});
                        }
                    }
                }
                java.awt.Graphics g = backBuffer.getGraphics();
                ...
                if (!model.paused)
                {
                    ...
                    long evolveTimeNanos = t1nanos-t0nanos;
                    model.evolveTimesNanos[view.nPaintsDone%model.evolveTimesNanos.length] = evolveTimeNanos;
                    //System.out.println(com.donhatchsw.compat.Format.sprintf("    evolve took %d.%03d secs.", (int)(evolveTimeNanos/1000000000), (int)(evolveTimeNanos % 1000000000)));
                }

                if (view.showTimingsFlag)
                {
                    // Show slightly different colored background at 60hz mark
                    // TODO: actually get draw timings, not just particular pieces of it, to make sure nothing falls through cracks
                    mg.setColor(new java.awt.Color(32,32,32));
                    g.fillRect(0, h-(int)(16.66666667*view.timingsPixelsPerMs),
                               w, h);
                }
                ...
                for (int i = (n)-1; (i) >= 0; --i) // faster first, so slower on top
                {
                    for (int j = 0; (j) < (3); ++j) // separable first, so try_to_do_it_right on top
                    //FORIDOWN (j, 3) // try_to_do_it_right first, so separable on top
                    {
                        ...
                        double a = 4*model.referenceTripDistancesInPixels[i]/10000.
                                   / ((model.referenceTripTimesInRepaints[i])*(model.referenceTripTimesInRepaints[i]));
                        ...
                        if (I >= N-view.nPlansToShow)
                        {
                            ...
                            int skip = 5;
                            int nTimestepsInFuture = 1;
                            //int nTimestepsInFuture = -11*skip; // step backwards too. cute but I think would be better to show actual history
                            while (((int)model.time + nTimestepsInFuture) % skip != 0)
                                nTimestepsInFuture++;
                            long t0nanos = System.nanoTime();
                            ...
                            long t1nanos = System.nanoTime();
                            model.planTimesNanos[view.nPaintsDone%model.planTimesNanos.length] += (t1nanos-t0nanos);
                ...
                {
                    int col = w-1;
                    for (int iPaint = view.nPaintsDone; iPaint >= 0 && col >= 0; --iPaint, --col)
                    {
                        long evolveTimeNanos = model.evolveTimesNanos[iPaint%model.evolveTimesNanos.length];
                        long planTimeNanos = model.planTimesNanos[iPaint%model.planTimesNanos.length];
                        if (evolveTimeNanos >= 1)
                        {
                            mg.setColor(java.awt.Color.RED);
                            g.drawLine(col, h-(int)(evolveTimeNanos/(1000000/view.timingsPixelsPerMs)),
                                       col, h-1);
                        }
                        if (planTimeNanos >= 1)
                        {
                            mg.setColor(java.awt.Color.BLUE);
                            g.drawLine(col, h-(int)(evolveTimeNanos/(1000000/view.timingsPixelsPerMs))-(int)(planTimeNanos/(1000000/view.timingsPixelsPerMs)),
                                       col, h-(int)(evolveTimeNanos/(1000000/view.timingsPixelsPerMs))-1);
                        }
                        if (false)
                        {
                            if (iPaint == view.nPaintsDone)
                            {
                                System.out.println(com.donhatchsw.compat.Format.sprintf("%d.%09d", (int)(evolveTimeNanos/1000000000), (int)(evolveTimeNanos%1000000000))
                                + " + "
                                + com.donhatchsw.compat.Format.sprintf("%d.%09d", (int)(planTimeNanos/1000000000), (int)(planTimeNanos%1000000000)));
                            }
                        }
                    }
                }
                ...
                //if (viewParams.showNumPaintsDone.get())
                {
                    ...
                    //mgTemp.drawString("("+view.nPaintsDone+" paint"+(viewState.nPaintsDone==1?"":"s")+")", W-2, 2, 1, -1.);
                    String timeString = ((double)(int)model.time==model.time
                                      ? ""+(int)model.time
                                      : ""+model.time);
                    mgTemp.drawString("t="+timeString, W-2, Y, 1, -1.);
                    Y += 18;
                    if (model.dumpGoodAndBadFlag)
                        mgTemp.drawString("dumping trace to file "+model.goodAndBadOutFileName, W-2, Y, 1, -1.);
                    Y += 18;
                }
            ...
            void tweakTarget(double dx, double dy)
            {
                double target[] = new double[2];
                for (int I = 0; (I) < (model.points.length); ++I)
                {
                    model.points[I].getTargetPosition(target);
                    target[0] += dx;
                    target[1] += dy;
                    model.points[I].setTargetPosition(target);
                }
                    ...
                    public void keyPressed(java.awt.event.KeyEvent e)
                    {
                        ...
                        else if (keyCode == java.awt.event.KeyEvent.VK_UP)
                        {
                            if (model.paused)
                            {
                                ...
                                model.evolve(1.);
                                long t1nanos = System.nanoTime();
                                long evolveTimeNanos = (int)(t1nanos-t0nanos);
                                model.evolveTimesNanos[view.nPaintsDone%model.evolveTimesNanos.length] = evolveTimeNanos;
                                System.out.println(com.donhatchsw.compat.Format.sprintf("    evolve took %d.%03d secs.", (int)(evolveTimeNanos/1000000000), (int)(evolveTimeNanos % 1000000000)));
                                repaint();
                            }
                        ...
                        else if (keyCode == java.awt.event.KeyEvent.VK_DOWN)
                        {
                            if (model.paused)
                            {
                                ...
                                long evolveTimeNanos = t1nanos-t0nanos;
                                model.evolveTimesNanos[view.nPaintsDone%model.evolveTimesNanos.length] = evolveTimeNanos;
                                System.out.println(com.donhatchsw.compat.Format.sprintf("    devolve took %d.%03d secs.", (int)(evolveTimeNanos/1000000000), (int)(evolveTimeNanos % 1000000000)));
                                repaint();
                            }
                            else
                                tweakTarget(0., -1/9.);
                        ...
                        else if (c == java.awt.event.KeyEvent.CHAR_UNDEFINED)
                        {
                            ...
                            //System.out.println("CHAR_UNDEFINED");
                              ...
                              || c == (('c')-'a'+1)) // XXX do I want this? should be control panel I think, if I ever make one for this demo3
                        {
                            System.out.println("ciao!");
                            System.exit(0); // asinine way of doing things
                        }
                        ...
                        {
                            model.dumpGoodAndBadFlag ^= true;
                            {System.out.print("        "); System.out.println("model.dumpGoodAndBadFlag" + " = " + (model.dumpGoodAndBadFlag));};
                            repaint();
                        }
                        else if (c == ' ')
                        {
                            model.paused = !model.paused;
                            {System.out.print("        "); System.out.println("model.paused" + " = " + (model.paused));};
                            if (!model.paused)
                              repaint();
                        }
                        else if (c == 'p')
                        {
                            view.nPlansToShow++;
                            {System.out.print("        "); System.out.println("view.nPlansToShow" + " = " + (view.nPlansToShow));};
                            repaint();
                        }
                        else if (c == (('s')-'a'+1))
                        {
                            view.showTimingsFlag ^= true;
                            {System.out.print("        "); System.out.println("view.showTimingsFlag" + " = " + (view.showTimingsFlag));};
                            repaint();
                        }
                        else if (c == 's')
                        {
                            if (view.showTimingsFlag) // too easy to accidentally mess with otherwise
                            {
                                view.timingsPixelsPerMs /= 2.;
                                {System.out.print("        "); System.out.println("view.timingsPixelsPerMs" + " = " + (view.timingsPixelsPerMs));};
                                repaint();
                            }
                        }
                        else if (c == 'S')
                        ...
                        {
                            view.showPlanAccelerationsFlag ^= true;
                            {System.out.print("        "); System.out.println("view.showPlanAccelerationsFlag" + " = " + (view.showPlanAccelerationsFlag));};
                            repaint();
                        }
                        else if (c == 'v')
                        {
                            ...
                            //repaint();
                            verboseLevel = (verboseLevel+1)%3;
                            catenaryVerboseLevel = verboseLevel;
                        ...
                        {
                            model.distanceBasedBounce += .05;
                            {System.out.print("        "); System.out.println("model.distanceBasedBounce" + " = " + (model.distanceBasedBounce));};
                            repaint();
                        }
                        else if (c == 'B')
                        {
                        ...
                        {
                            System.out.println("The following keys do things:");
                            System.out.println("            b/B - increment/decrement distanceBasedBounce by .05");
                            System.out.println("              h - show this help message");
                            System.out.println("            p/P - increase/decrease number of plan curves to show");
                            System.out.println("              a - toggle accelerations on plan curves");
                            System.out.println("         ctrl-s - toggle timing graph");
                            System.out.println("            s/S - decrease/increase timing graph size");
                            System.out.println("              d - toggle tracing to "+model.goodAndBadOutFileName);
                            System.out.println("          space - pause/resume");
                            System.out.println("         arrows - move target by 1/9 pixel if not paused");
                            System.out.println("  up/down arrow - advance/retreat one timestep if paused");
                ...
                addMouseListener(new java.awt.event.MouseListener() {
                    ...
                    public void mousePressed(java.awt.event.MouseEvent e)
                    {
                        ...
                        if (mostRecentGraphics != null)
                        {
                            ...
                            for (int I = 0; (I) < (model.points.length); ++I)
                                model.points[I].setTargetPosition(thisPoint);
        ...
        frame.pack();
        frame.setVisible(true); // available in java 1.5, replaces deprecated show()
        CatenaryRotated.XXX_ALLOW_FITFROMMOMENT_FAILURE = true; // hack so we can run with manageable amount of noise (1 line per failure), for now
    } // interactiveDemo3
...
(note, learned a lot since I wrote the above, need to include corresponce)
