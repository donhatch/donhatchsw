...
// !!! THIS FILE WAS AUTOMATICALLY GENERATED BY javacpp; DO NOT EDIT !!!
// !!! THIS FILE WAS AUTOMATICALLY GENERATED BY javacpp; DO NOT EDIT !!!
// 102 # 1 "com/donhatchsw/util/MiniBall.prejava"
// 103 # 1 "<built-in>"
// 104 # 1 "<command-line>"
// 105 # 1 "com/donhatchsw/util/MiniBall.prejava"
/* vim: set filetype=java: */
package com.donhatchsw.util;

// 109 # 1 "com/donhatchsw/util/macros.h" 1
//
// macros.h
//
// 113 # 14 "com/donhatchsw/util/macros.h"
// NOTE: this causes a,b to be evaluated twice on failure, so is not ideal. might be better to do a block... ?
// 115 # 30 "com/donhatchsw/util/macros.h"
// Note: the additional "(a)==(b)||" is to make it work correctly for infinities...
// and causes the args to be evaluated twice.
// 118 # 67 "com/donhatchsw/util/macros.h"
// XXX ... what a mess
// 120 # 5 "com/donhatchsw/util/MiniBall.prejava" 2

/*
TODO: retain record of which support points were used to compute the ball?
TODO: think about whether the fuzzy outside test should be necessary
*/
                                     ...
                                     double points[][])
    {
        if (verboseLevel >= 1)
        {
            ...
            if (verboseLevel >= 2)
            {
                ...
                System.out.println("        " + "center" + " = " + Arrays.toStringCompact(center));
                {System.out.print("        "); System.out.println("radiusSquared" + " = " + (radiusSquared));};
                double radius = Math.sqrt(radiusSquared);
                {System.out.print("        "); System.out.println("radius" + " = " + (radius));};
            }
        }
        ...
        int nOn = 0;
        int nOut = 0;
        int on[] = new int[nPoints];
        for (int iPoint = 0; (iPoint) < (nPoints); ++iPoint)
        {
            double dist = VecMath.dist(points[iPoint], center);
            //PRINT(dist);
            //PRINT(dist-radius);
            if ((((dist)-(radius)) > (radius*1e-12)))
                nOut++;
            else if ((((radius)-(dist)) > (radius*1e-12)))
                nIn++;
            else
                on[nOn++] = iPoint;
        }
        if (verboseLevel >= 1)
        {
            System.out.print("    "+nOn+" on: "+Arrays.toStringCompact(Arrays.subarray(on,0,nOn)));
            System.out.print(", ");
        ...
        if (dim == 0)
        {
            do { if (!((nOn)==(nPoints))) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+87 +"): (" + "nOn" + ")" + "==" + "(" + "nPoints" + ") ("+(nOn)+" vs. "+(nPoints)+")"); } while (false);
            if (verboseLevel >= 1)
        ...
        {
            do { if (!(nOn > 0)) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+95 +"): " + "nOn > 0" + ""); } while (false);

            {
                // reduce nOn to a set of dim+1 support points
                // (or fewer, if dimension spanned by the support points is less than that)
                int supportInds[] = new int[dim+1];
                ...
                for (i = 0; (i) < (dim+1); ++i)
                {
                    ...
                    {
                        supportInds[i] = on[j];
                        supportPoints[i] = points[supportInds[i]];
                        double thisContent = VecMath.relativeOrthotopeContent(i+1, dim, supportPoints, false);
                        thisContent = ((thisContent) < 0 ? -(thisContent) : (thisContent));
                        //PRINT(thisContent);
                    ...
                    do { if (!(bestJ != -1)) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+125 +"): " + "bestJ != -1" + ""); } while (false);
                    supportInds[i] = on[bestJ];
                    supportPoints[i] = points[supportInds[i]];
                    //PRINT(i);
                    if (verboseLevel >= 1)
                        {System.out.print("        "); System.out.println("bestContent" + " = " + (bestContent));};
                ...
                do { if (!(i > 0)) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+134 +"): " + "i > 0" + ""); } while (false);
                nOn = i;
                on = supportInds;
                if (verboseLevel >= 1)
                    System.out.println("nOn reduced to "+nOn+" affinely independent");
            }

            do { if (!(nOn <= dim+1)) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+141 +"): " + "nOn <= dim+1" + ""); } while (false);
            ...
            double M[][] = new double[nOn][dim];
            // XXX the following may be silly, think about it
            for (int iOn = 0; (iOn) < (nOn); ++iOn) for (int iDim = 0; (iDim) < (dim); ++iDim) M[iOn][iDim] = points[on[iOn]][iDim];
            double m[][] = new double[nOn-1][dim]; // scratch
            for (int iRow = 0; (iRow) < (nOn-1); ++iRow)
            ...
            fulldet = ((fulldet) < 0 ? -(fulldet) : (fulldet));
            double absBarys[] = new double[nOn];
            for (int iOn = 0; (iOn) < (nOn); ++iOn)
            {
                // XXX the following may be silly, think about it
                for (int _iOn = 0; (_iOn) < (nOn); ++_iOn) for (int iDim = 0; (iDim) < (dim); ++iDim) M[_iOn][iDim] = points[on[_iOn]][iDim];
            ...
            if (!((((VecMath.sum(absBarys))-(1.)) <= (1e-6)) && (((1.)-(VecMath.sum(absBarys))) <= (1e-6))))
            {
                {System.out.print("        "); System.out.println("VecMath.sum(absBarys)" + " = " + (VecMath.sum(absBarys)));};
                System.out.println("        " + "absBarys" + " = " + Arrays.toStringCompact(absBarys));
                if (verboseLevel >= 1)
    ...
    */
                                  ...
                                  double points[][])
    {
        ...
        double radiusSquared = MiniBallHelper(resultCenter,
                                              ...
                                              0, new int[((points.length)<=(dim+1)?(points.length):(dim+1))],
                                              0);
        if (checkEveryMiniBall)
        {
            do { if (!(IsMiniBall(resultCenter, radiusSquared, points.length, points))) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+209 +"): " + "IsMiniBall(resultCenter, radiusSquared, points.length, points)" + ""); } while (false);
        }
        if (verboseLevel >= 1)
                                         ...
                                         int recursionLevel)
    {
        if (verboseLevel >= 2)
        {
            for (int iIndent = 0; (iIndent) < (recursionLevel); ++iIndent) System.out.print("    ");
        ...
        for (int i = 0; (i) < (nL); ++i)
        {
            ...
            if (verboseLevel >= 3)
            {
                System.out.println("        " + "resultCenter" + " = " + VecMath.toString(resultCenter));
                {System.out.print("        "); System.out.println("radiusSquared" + " = " + (radiusSquared));};
                {System.out.print("        "); System.out.println("VecMath.distsqrd(points[Li], resultCenter)" + " = " + (VecMath.distsqrd(points[Li], resultCenter)));}
                {System.out.print("        "); System.out.println("Math.sqrt(radiusSquared)" + " = " + (Math.sqrt(radiusSquared)));};
            }
            if (radiusSquared < 0
             || (((VecMath.distsqrd(points[Li], resultCenter))-(radiusSquared)) > (1e-12 * radiusSquared)))
             // XXX wait, should the fuzziness be necessary? hmm
            {
                if (verboseLevel >= 2)
                {
                    for (int iIndent = 0; (iIndent) < (recursionLevel); ++iIndent) System.out.print("    ");
                ...
                for (int iiL = 0; iiL < nL; ++iiL)
                {
                    if we want to move iL to front:
                    {
                        ...
                        {
                            CHECK_EQ(nextInOrder[iLprev], iL);
                                     ...
                                     int inds[])
    {
        ...
        int dim = resultCenter.length;
        do { if (!(nInds <= dim+1)) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+364 +"): " + "nInds <= dim+1" + ""); } while (false);
        if (nInds == 0)
        {
...
// 495 # 406 "com/donhatchsw/util/MiniBall.prejava"
        else if (true)
        {
            ...
            for (int i = 0; (i) < (Qlength2s.length); ++i)
            ...
            if (assertBarysNonNegative)
            {
                ...
                for (int i = 0; (i) < (twice_lambdas.length); ++i)
                    barys[i+1] = lambdas[i];

                if ((((0.)-(VecMath.min(barys))) > (1e-12)))
                {
                    /*
                    ...
                    */
                    ...
                    System.out.println("        " + "barys" + " = " + VecMath.toString(barys));
                    System.out.println("=================================");
        ...
        {
            do { if (!(false)) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+594 +"): " + "false" + ""); } while (false);
            return 0.;
        }
    } // CircumBall
    ...
    public static void main(String args[])
    {
        ...
        if (false)
        {
            ...
            if (!new java.io.File(cacheFileName).exists())
            {
                ...
                long b = Math.round(Math.sqrt(l2));
                for (long a = 0; a*a + a*a <= l2; ++a)

                //double b = Math.round(Math.sqrt(l2));
                //for (double a = 0; a*a + a*a <= l2; ++a)

                {
                    //b = Math.round(Math.sqrt(l2 - a*a));
                    // much quicker way to get at it...
                    while (a*a + b*b > l2 && b >= 0) --b;
                    if (a*a + b*b == l2)
                    {
                        do { if (!(0 < a)) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+642 +"): " + "0 < a" + ""); } while (false);
                        do { if (!(a < b)) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+643 +"): " + "a < b" + ""); } while (false);
                ...
                {System.out.print("        "); System.out.println("nFound" + " = " + (nFound));};
                do { if (!((nFound)==(nExpected))) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+656 +"): (" + "nFound" + ")" + "==" + "(" + "nExpected" + ") ("+(nFound)+" vs. "+(nExpected)+")"); } while (false);
                do { if (!((nFound)==(points.length))) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+657 +"): (" + "nFound" + ")" + "==" + "(" + "points.length" + ") ("+(nFound)+" vs. "+(points.length)+")"); } while (false);
                ...
                int intPoints[][] = new int[points.length][points[0].length];
                {
                    for (int iPoint = 0; (iPoint) < (points.length); ++iPoint)
                    {
                        ...
                        do { if (!(((double)x)==(points[iPoint][0]))) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+666 +"): (" + "(double)x" + ")" + "==" + "(" + "points[iPoint][0]" + ") ("+((double)x)+" vs. "+(points[iPoint][0])+")"); } while (false);
                        do { if (!(((double)y)==(points[iPoint][1]))) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+667 +"): (" + "(double)y" + ")" + "==" + "(" + "points[iPoint][1]" + ") ("+((double)y)+" vs. "+(points[iPoint][1])+")"); } while (false);
                        intPoints[iPoint][0] = x;
                        intPoints[iPoint][1] = y;
                    }
                }
                System.out.println("        " + "intPoints" + " = " + Arrays.toStringCompact(intPoints));
            }
            else // read it from cache
            {
                ...
                catch (java.io.IOException e)
                {
                    ...
                    do { if (!(false)) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+690 +"): " + "false" + ""); } while (false);
                }
                if (false)
                {
                    try {
                    ...
                    catch (java.text.ParseException e)
                    {
                        ...
                        do { if (!(false)) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+700 +"): " + "false" + ""); } while (false);
                    }
                }
                else
                {
                    ...
                    // XXX my regex doesn't handle \s inside [] compatibly, apparently
                    // XXX huh? how come there's an empty string at the beginning but not at the end?? this is completely retarded
                    System.out.println("        " + "tokens[0]" + " = " + Arrays.toStringCompact(tokens[0]));
                    System.out.println("        " + "tokens[tokens.length-1]" + " = " + Arrays.toStringCompact(tokens[tokens.length-1]));
                    do { if (!((tokens.length % 2)==(1))) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+712 +"): (" + "tokens.length % 2" + ")" + "==" + "(" + "1" + ") ("+(tokens.length % 2)+" vs. "+(1)+")"); } while (false);
                    do { if (!(tokens[0].equals(""))) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+713 +"): " + "tokens[0].equals(\"\")" + ""); } while (false);
                    do { if (!(tokens[tokens.length-1] != "")) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+714 +"): " + "tokens[tokens.length-1] != \"\"" + ""); } while (false);
            ...
            {System.out.print("        "); System.out.println("radius" + " = " + (radius));};
            for (int iPoint = 0; (iPoint) < (points.length); ++iPoint)
                {System.out.print("        "); System.out.println("VecMath.dist(center, points[iPoint])" + " = " + (VecMath.dist(center, points[iPoint])));};
            {System.out.print("        "); System.out.println("VecMath.dist(points[0],points[1])" + " = " + (VecMath.dist(points[0],points[1])));};
            {System.out.print("        "); System.out.println("VecMath.dist(points[1],points[2])" + " = " + (VecMath.dist(points[1],points[2])));};
            {System.out.print("        "); System.out.println("VecMath.dist(points[2],points[0])" + " = " + (VecMath.dist(points[2],points[0])));};
        ...
        if (true)
        {
            double examples[][][] = {
                ...
                {
                    // 3 (identical) points in 0d
                    {},{},{},
                },
                {
                    // 3 identical points in 1d
                    {-.1},{-.1},{-.1},
                },
                {
                    // 3 (2 same) points in 1d
                    {-.1},{-.1},{-.2},
                },
                {
                    // 3 diff points in 1d
                ...
                {
                    // 3 identical points in 2d
                    {-.1,-.1},{-.1,-.1},{-.1,-.1},
                },
                {
                    // 3 (2 same) points in 2d
                    {-.1,-.1},{-.1,-.1},{-.2,-.3},
                },
                {
                    // 3 diff points in 1d
                ...
                {
                    // 4 (3 same) points in 2d
                    {-.1,-.1},{-.1,-.1},{-.1,-.1},{-.2,-.3},
                },
                {
                    // one that was giving me an assertion failure in ShephardsPlayApplet
                    {-0.28,-0.04},
                    {-0.28,-0.04},
                    {-0.28,-0.04},
                    {0.2777777777777778,0.3},
                    {0.015625,-0.375},
                },
                {
                    // one that was giving me an assertion failure in ShephardsPlayApplet, with mistaken 3rd coord added
                    {-0.28116531165311653,-0.04067796610169491,0.0},
                    {-0.28116531165311653,-0.04067796610169491,0.0},
                    {-0.28116531165311653,-0.04067796610169491,0.0},
                    {0.2777777777777778,0.3016949152542373,0.0},
                    {0.015625,-0.375,0.0},
                },
            };
            for (int iExample = 0; (iExample) < (examples.length); ++iExample)
            {
        ...
        if (true)
        {
            ...
            for (int iPoint = 0; (iPoint) < (points.length); ++iPoint)
                {System.out.print("        "); System.out.println("VecMath.dist(center, points[iPoint])" + " = " + (VecMath.dist(center, points[iPoint])));};
            for (int iPoint = 0; (iPoint) < (points.length); ++iPoint)
                {System.out.print("        "); System.out.println("VecMath.dist(center, points[iPoint]) - radius" + " = " + (VecMath.dist(center, points[iPoint]) - radius));};
            //return;
        }
        if (true)
        {
            ...
            for (int nPoints = minPoints; nPoints <= maxPoints; nPoints++)
            {
                ...
                for (int iTest = 0; (iTest) < (nTests); ++iTest)
                {
                    ...
                    for (int iPerm = 0; (iPerm) < (nPerms); ++iPerm)
                    {
                        ...
                        if (verboseLevel >= 1)
                        {
                            System.out.println("        " + "nPoints" + " = " + Arrays.toStringCompact(nPoints));
                            System.out.println("        " + "points" + " = " + Arrays.toStringCompact(points));
                        }
                        ...
                        if (verboseLevel >= 1)
                        {
                            System.out.println("        " + "resultCenter" + " = " + Arrays.toStringCompact(resultCenter));
                            {System.out.print("        "); System.out.println("resultRadius" + " = " + (resultRadius));};
                        }
                        if (iPerm == 0)
                        {
                        ...
                        {
                            do { if (!(VecMath.equals(resultCenter, expectedResultCenter, 1e-6))) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+1119 +"): " + "VecMath.equals(resultCenter, expectedResultCenter, 1e-6)" + ""); } while (false);
                            do { if (!(((((Math.sqrt(resultRadiusSquared))-(Math.sqrt(expectedRadiusSquared))) <= (1e-6)) && (((Math.sqrt(expectedRadiusSquared))-(Math.sqrt(resultRadiusSquared))) <= (1e-6))))) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+1120 +"): " + "EQ(Math.sqrt(resultRadiusSquared), Math.sqrt(expectedRadiusSquared), 1e-6)" + ""); } while (false)
                                                                             ;
                        }
                        do { if (!(IsMiniBall(resultCenter, resultRadiusSquared, points.length, points))) throw new Error("CHECK failed at "+"com/donhatchsw/util/MiniBall.prejava"+"("+1123 +"): " + "IsMiniBall(resultCenter, resultRadiusSquared, points.length, points)" + ""); } while (false);
