...
// !!! THIS FILE WAS AUTOMATICALLY GENERATED BY javacpp; DO NOT EDIT !!!
// !!! THIS FILE WAS AUTOMATICALLY GENERATED BY javacpp; DO NOT EDIT !!!
// 102 # 1 "com/donhatchsw/util/UndoTreeSquirrel.prejava"
// 103 # 1 "<built-in>"
// 104 # 1 "<command-line>"
...
// 150 # 1 "com/donhatchsw/util/UndoTreeSquirrel.prejava"

// 152 # 1 "com/donhatchsw/util/UndoTreeSquirrel.prejava"
/* vim: set filetype=java: */
package com.donhatchsw.util;

import com.donhatchsw.compat.regex;

// 158 # 1 "com/donhatchsw/util/macros.h" 1
//
// macros.h
//
// 162 # 14 "com/donhatchsw/util/macros.h"
// NOTE: this causes a,b to be evaluated twice on failure, so is not ideal. might be better to do a block... ?
// 164 # 30 "com/donhatchsw/util/macros.h"
// Note: the additional "(a)==(b)||" is to make it work correctly for infinities...
// and causes the args to be evaluated twice.
// 167 # 67 "com/donhatchsw/util/macros.h"
// XXX ... what a mess
// 169 # 7 "com/donhatchsw/util/UndoTreeSquirrel.prejava" 2
...
* <p>
* An UndoTreeSquirrel represents the notion of a current position
* in an undo tree.  It can travel up and down the tree (undo and redo),
* and it also has a memory of where it's been;
* when given a choice of possible redo branches, it will choose
* the branch it was most recently in.
* <p>
* Multiple squirrels can live in the same undo tree,
* and one squirrel can be told to follow another, possibly
* at a limited speed.
* This can be useful in Model/View/Controller scenario
* in which the Controller's notion of current state
* is allowed to change very fast or instantaneously,
* while the View's animation lags behind, progressing continuously
* at a slower speed.
* This scenario would be represented by two squirrels
* living in the same tree: the Controller squirrel
* moves around fast and can create new branches,
* while the View squirrel follows the Controller squirrel around,
* always moving towards it.
* <p>
* Currently there is no way for an application to have a direct
* handle to an undo tree; undo trees are only accessed and manipulated
* through squirrels.  This includes growing the tree: the only way
* to add nodes to a tree is by the actions of a squirrel
* (the Do method).
* XXX no longer true
*/

public class UndoTreeSquirrel
{
    ...
    /** A listening callback that you can create and install using addListener to listen for changes to the UndoTreeSquirrel. Hears changes to both the shared model (the actual tree, which other views (UndoTrees) might be listening to) and your individual view (i.e. current position and private node labels). */
    public interface Listener
    {
        public void somethingChanged();
    }

    // It's a model/view setup, where UndoTreeSquirrel itself is the view and UndoTreeSquirrel.UndoTree is the model (the tree itself).
    public static class UndoTree
    {
        ...
        // (although that is a Squirrel concept, since the model
        ...
        public void Clear()
        {
            ...
            {
                nodes.clear();
                nodes.add(new UndoTree.Node(-1,null,System.currentTimeMillis()));
            }
        ...
        {
            if (item == null)
                throw new IllegalArgumentException("UndoTreeSquirrel.Do called with null item");
            ...
            parentNode.children = Arrays.append(parentNode.children, newChildIndex-parentNodeIndex);
            notifyListenersSomethingChanged(modelListeners);
    ...
    private UndoTree model; // set in ctor

    private static PrivateNodeInfo getOrMakePrivateNodeInfo(java.util.HashMap<UndoTree.Node,PrivateNodeInfo> privateNodeInfosHashMap, UndoTree.Node node)
    {
        PrivateNodeInfo info = privateNodeInfosHashMap.get(node);
        if (info == null)
        {
            info = new PrivateNodeInfo();
            privateNodeInfosHashMap.put(node, info);
        }
        return info;
    } // PrivateNodeInfo
    ...
    //
    // An UndoTreeSquirrel is actually a view into the nodes (the model).
    ...
    private java.util.HashMap<UndoTree.Node,PrivateNodeInfo> privateNodeInfosHashMap = new java.util.HashMap<UndoTree.Node,PrivateNodeInfo>(); // XXX get rid of this, make it an array instead?  not sure
    private int currentNodeIndex; // set in ctor
    private SmoothlyVaryingViewingParameter currentNodeFraction = new SmoothlyVaryingViewingParameter(1.); // current fraction of the way from parent to current node. Usually in (0,1] but can be outside that range in the case of a bouncy system.
    ...
    {
        synchronized(privateNodeInfosHashMap)
        {
            ...
            // Correct the preferred child path from the root // XXX this is more costly than it should be!  there should be an O(1) way to do this! maybe
            {
                ...
                UndoTree.Node child = (UndoTree.Node)model.nodes.get(iChild);

                while (iChild != 0)
                {
                    int iParent = iChild + child.parent;
                    UndoTree.Node parent = (UndoTree.Node)model.nodes.get(iParent);
                    int childIndex = Arrays.indexOf(parent.children, -child.parent);
                    PrivateNodeInfo parentInfo = getOrMakePrivateNodeInfo(privateNodeInfosHashMap, parent);
    ...
    public Object getItemOnEdgeFromParentToCurrentNode() // XXX definitely shouldn't have to be public... root out the usages of it and see if we can handle it a different way
    {
        ...
        {
            return ((UndoTree.Node)model.nodes.get(currentNodeIndex)).itemOnEdgeFromParentToMe;
        }
    }
    ...
    * XXX fix this, wrong now
    */
    // CTOR    (for quick find in editor)
    public UndoTreeSquirrel(UndoTreeSquirrel other)
    {
        this(other==null ? null : other.model); // XXX probably don't need this any more
        if (other != null)
        {
            this.model = other.model;
            this.currentNodeIndex = other.currentNodeIndex;
            this.currentNodeFraction.setTargetPosition(other.currentNodeFraction.getTargetPosition());
    ...
    public UndoTreeSquirrel(UndoTree tree)
    {
        if (tree == null)
        {
            this.model = new UndoTree();
        }
        else
        {
            this.model = tree;
        }
        this.currentNodeIndex = 0; // XXX might an existing tree have this pruned away?
        this.currentNodeFraction.setTargetPosition(1.);
        this.currentNodeFraction.snap(1.);
        ...
        this.model.addListener(new Listener() { // XXX this never gets removed!  should it be a weak reference or something?
            ...
            {
                if (UndoTreeSquirrel.this.model.nodes.size() == 1) // it's always at least 1
                {
                    ...
                    privateNodeInfosHashMap.clear();
                }
                do { if (!(currentNodeIndex < UndoTreeSquirrel.this.model.nodes.size())) throw new Error("CHECK failed at "+"com/donhatchsw/util/UndoTreeSquirrel.prejava"+"("+296 +"): " + "currentNodeIndex < UndoTreeSquirrel.this.model.nodes.size()" + ""); } while (false); // XXX will need to do something more sophisticated if pruning is implemented; for now, the only destructive operation is Clear which we just handled
    ...
    } // UndoTreeSquirrel (view) ctor

    /** Creates a new undo tree (view) with a brand new empty tree; equivalent to UndoTreeSquirrel(null). */
    public UndoTreeSquirrel()
    {
        this((UndoTree)null);
    }
    ...
    */
    public void Do(Object item, long timeStampMillis)
    {
        ...
        synchronized(this) // note currentNodeIndex can change while waiting for this lock!  that will make us hang it off the new currentNodeIndex instead.  that's the way the ball bounces.
        {
            ...
            synchronized(model) // currentNodeIndex can also change while waiting for *this* lock, since if someone is clearing the model, that will notify us and *we* will set it to zero.  that's the way the ball bounces.
            {
                ...
                PrivateNodeInfo currentNodeInfo = getOrMakePrivateNodeInfo(privateNodeInfosHashMap, (UndoTree.Node)model.nodes.get(currentNodeIndex));
    ...
    public Object undo()
    {
        ...
        {
            UndoTree.Node currentNode = (UndoTree.Node)model.nodes.get(currentNodeIndex);
    ...
    public Object redo()
    {
        ...
        if (currentNode.children.length == 0)
            return null;
        PrivateNodeInfo currentNodeInfo = getOrMakePrivateNodeInfo(privateNodeInfosHashMap, currentNode);
        ...
        currentNode = (UndoTree.Node)model.nodes.get(currentNodeIndex);
        currentNodeFraction.snap(1.);
        notifyListenersSomethingChanged(viewListeners);
    ...
    */
    ...
    public void setPrivateLabel(Object privateLabel)
    {
        ...
        {
            UndoTree.Node currentNode = (UndoTree.Node)model.nodes.get(currentNodeIndex);
            PrivateNodeInfo privateNodeInfo = getOrMakePrivateNodeInfo(privateNodeInfosHashMap, currentNode);
    ...
    */
    public boolean isMoving(UndoTreeSquirrel target)
    {
    ...
    * XXX that doesn't seem to be true any more... doesn't return the fractional offset, just stores it
    */
    // XXX restructure this function to make it clearer
    public ReturnValueOfIncrementViewTowardsOtherView incrementViewTowardsOtherView(UndoTreeSquirrel otherView,
                                                 double referenceTripDistance,
                                                 double referenceTripTime,
                                                 double dt,
                                                 double bounce,
                                                 UndoTreeSquirrel.ItemLengthizer lengthizer)
    {
        boolean debug = false; // manually set to true to debug this function
        if (debug) System.out.println("in incrementViewTowardsOtherView");
        if (otherView.model.nodes != model.nodes)
            throw new IllegalStateException("UndoTreeSquirrel.incrementViewTowardsOtherView called with a different underlying tree!");
        ...
        if (currentNodeIndex == targetNodeIndex)
        {
            ...
            currentNodeFraction.setTargetPosition(targetNodeFraction); // if it wasn't already
            if (!currentNodeFraction.isMoving())
            {
                if (debug) System.out.println("        already there");
                if (debug) System.out.println("out incrementViewTowardsOtherView (no change)");
                return null;
            }
            //
            ...
            //
            if (debug) System.out.println("        evolving "+currentNodeFraction.get()+" -> "+currentNodeFraction.getTargetPosition()+"");
            ...
            currentNodeFraction.evolve(referenceTripFraction,
                                       ...
                                       bounce);
            notifyListenersSomethingChanged(viewListeners);
            if (debug) System.out.println("out incrementViewTowardsOtherView (no discrete change)");
        ...
        if (ancestorIndex == currentNodeIndex)
        {
            ...
            currentNodeFraction.setTargetPosition(1.); // if it wasn't already
            if (!currentNodeFraction.isMoving())
            {
                ...
                PrivateNodeInfo currentNodeInfo = getOrMakePrivateNodeInfo(privateNodeInfosHashMap, currentNode);
                synchronized(otherView.privateNodeInfosHashMap) // XXX I'm positive this is the wrong thing
                {
                    PrivateNodeInfo currentNodeInfoInOtherView = otherView.privateNodeInfosHashMap.get(currentNode);
                ...
                do { if (!((currentNodeFraction.get())==(0.))) throw new Error("CHECK failed at "+"com/donhatchsw/util/UndoTreeSquirrel.prejava"+"("+561 +"): (" + "currentNodeFraction.get()" + ")" + "==" + "(" + "0." + ") ("+(currentNodeFraction.get())+" vs. "+(0.)+")"); } while (false);
                currentNode = (UndoTree.Node)model.nodes.get(currentNodeIndex);
            ...
            else // currentNodeFraction.isMoving()
            {
                if (debug) System.out.println("        evolving "+currentNodeFraction.get()+" -> "+currentNodeFraction.getTargetPosition()+"");
                ...
                // at the transition.
                // (XXX need a smarter transition mechanism... !isMoving won't cut it any more)  (XXX what did I mean by this??)
                //
                if (true)
                {
                    double currentItemLength = lengthizer.length(currentNode.itemOnEdgeFromParentToMe);

                    int nodeIndex = currentNodeIndex;
                    UndoTree.Node node = currentNode;
                    Object item = currentNode.itemOnEdgeFromParentToMe;
                    ...
                    while (true)
                    {
                        ...
                        UndoTree.Node childNode = (UndoTree.Node)model.nodes.get(childIndex); // XXX synch!?
                        ...
                        //
                        // Advance the target farther.
                        //
                        currentNodeFraction.setTargetPosition(currentNodeFraction.getTargetPosition()
                                + (lengthizer.length(childItem)
                                 / currentItemLength));

                        //System.out.println("            power -> "+currentNodeFraction.getTargetPosition()+" baby!");
                    ...
                    if (!currentNodeFraction.isMoving())
                    {
                        ...
                        // since we WERE moving originally before we started
                        // glomming on additional segments.
                        // but this will happen
                        // if we get all the way to the target
                        // without doing the switcheroo for some reason.
                ...
                // Switch rails here!
                //
                if (currentNodeFraction.getTargetPosition() > 1.
                 && currentNodeFraction.get() > 1.)
                {
                    ...
                    currentNodeFraction.setPosition(currentNodeFraction.get()-1.); // i.e. change position without changing velocity   XXX in more general circumstances, we may have to rescale the velocity, although this is fine for now since we only merge when consecutive edges are identical
                    do { if (!((currentNode.children.length)==(1))) throw new Error("CHECK failed at "+"com/donhatchsw/util/UndoTreeSquirrel.prejava"+"("+655 +"): (" + "currentNode.children.length" + ")" + "==" + "(" + "1" + ") ("+(currentNode.children.length)+" vs. "+(1)+")"); } while (false); // XXX for now
                    currentNodeIndex += currentNode.children[0]; // XXX should be preferred child index
                    currentNode = (UndoTree.Node)model.nodes.get(currentNodeIndex);
                    do { if (!((returnVal)==(null))) throw new Error("CHECK failed at "+"com/donhatchsw/util/UndoTreeSquirrel.prejava"+"("+658 +"): (" + "returnVal" + ")" + "==" + "(" + "null" + ") ("+(returnVal)+" vs. "+(null)+")"); } while (false); // there shouldn't be any other discrete change we're trying to return
                    returnVal = new ReturnValueOfIncrementViewTowardsOtherView(1, currentNode.itemOnEdgeFromParentToMe/*AFTER we change current node*/);
                }
            }
        }
        else // ancestorIndex != currentNodeIndex
        {
            // Go up in the undo direction
            if (debug) System.out.println("    going in undo direction");
            currentNodeFraction.setTargetPosition(0.); // if it wasn't already
            if (!currentNodeFraction.isMoving())
            {
                if (debug) System.out.println("        already there, transitioning");
                ...
                currentNodeFraction.snap(1.);
                do { if (!((currentNodeFraction.get())==(1.))) throw new Error("CHECK failed at "+"com/donhatchsw/util/UndoTreeSquirrel.prejava"+"("+674 +"): (" + "currentNodeFraction.get()" + ")" + "==" + "(" + "1." + ") ("+(currentNodeFraction.get())+" vs. "+(1.)+")"); } while (false);
                currentNode = (UndoTree.Node)model.nodes.get(currentNodeIndex);
                // In this case don't evolve... wait for next time around,
                // which will do something or other depending
                // on whether the new one is the target or not.
            ...
            {
                //
                // Do the combining-consecutive-identical-targets-
                // into-one-long-target thing.
                // Doing this here in the undo case is easier
                // than in the redo case since there is only one choice
                // of parent.

                if (true)
                {
                    Object currentItem = currentNode.itemOnEdgeFromParentToMe;
                    double currentItemLength = lengthizer.length(currentItem); // redundant calculation, maybe move this to a more outer scope

                    // Immediate target starts as the parent,
                    // but we might make it the grandparent, etc.
                    int immediateTargetNodeIndex = currentNodeIndex+currentNode.parent;
                    UndoTree.Node immediateTargetNode = (UndoTree.Node)model.nodes.get(immediateTargetNodeIndex);

                    //System.out.println("    Should I add power?");
                    while (true)
                    {
                        if (immediateTargetNodeIndex == ancestorIndex)
                        {
                            // too hairy to try to do this over the ancestor transition, so don't bother for now
                            //System.out.println("        no, at ancestor hump");
                            break;
                        }
                        if (!immediateTargetNode.itemOnEdgeFromParentToMe.equals(currentItem))
                        {
                            //System.out.println("        no, child item is different from this item");
                            break;
                        }
                        //
                        // Advance the immediate target farther.
                        //
                        currentNodeFraction.setTargetPosition(currentNodeFraction.getTargetPosition()
                                - (lengthizer.length(immediateTargetNode.itemOnEdgeFromParentToMe)
                                 / currentItemLength));
                        immediateTargetNodeIndex += immediateTargetNode.parent;
                        immediateTargetNode = (UndoTree.Node)model.nodes.get(immediateTargetNodeIndex);
                        //System.out.println("            power -> "+currentNodeFraction.getTargetPosition()+" baby!");
                        //System.out.println("        should I add more?");
                    }
                    if (!currentNodeFraction.isMoving())
                ...
                // Switch rails here!
                //
                if (currentNodeFraction.getTargetPosition() < 0.
                 && currentNodeFraction.get() < 0.)
                {
                    // Just do one switch even though potentially get()
                    // might be < -1, since we are only capable of returning
                    // one discrete state change to the caller.
                    // We'll get the next one next time around (hopefully).
                    //System.out.println("SWITCHING HORSES, BABY!");
                    currentNodeFraction.setPosition(currentNodeFraction.get()+1.); // i.e. change position without changing velocity   XXX in more general circumstances, we may have to rescale the velocity, although this is fine for now since we only merge when consecutive edges are identical

                    do { if (!((returnVal)==(null))) throw new Error("CHECK failed at "+"com/donhatchsw/util/UndoTreeSquirrel.prejava"+"("+756 +"): (" + "returnVal" + ")" + "==" + "(" + "null" + ") ("+(returnVal)+" vs. "+(null)+")"); } while (false); // there shouldn't be any other discrete change we're trying to return
                    returnVal = new ReturnValueOfIncrementViewTowardsOtherView(-1, currentNode.itemOnEdgeFromParentToMe/*BEFORE we change current node*/); // tell caller there was a discrete undo
                    currentNodeIndex += currentNode.parent;
                    currentNode = (UndoTree.Node)model.nodes.get(currentNodeIndex);
                }
            ...
            //
            // Hack for beautiful smooth transition in the undo tree viewer
            // as we turn around.
            ...
            if (currentNodeIndex != 0)
            {
                UndoTree.Node parentNode = (UndoTree.Node)model.nodes.get(currentNodeIndex+currentNode.parent);
                PrivateNodeInfo parentNodeInfo = getOrMakePrivateNodeInfo(privateNodeInfosHashMap, parentNode);
                synchronized(otherView.privateNodeInfosHashMap) // XXX I'm positive this is the wrong thing
                {
                    PrivateNodeInfo parentNodeInfoInOtherView = otherView.privateNodeInfosHashMap.get(parentNode);
        ...
        notifyListenersSomethingChanged(viewListeners);
        if (debug) System.out.println("out incrementViewTowardsOtherView");
        return returnVal;
    ...
    private static int findLowestCommonAncestorIndex(int i0, int i1, com.donhatchsw.compat.ArrayList nodes)
    {
        ...
        if (i0 == i1)
            return i0;
        if (i0 != 0 && i0+((UndoTree.Node)nodes.get(i0)).parent == i1)
            return i1;
        if (i1 != 0 && i1+((UndoTree.Node)nodes.get(i1)).parent == i0)
            return i0;

        // No clue where they are with respect to each other.
        // Figure out the depths...
        int depth0 = 0;
        for (int i = i0; i > 0; i += ((UndoTree.Node)nodes.get(i)).parent)
            depth0++;
        int depth1 = 0;
        for (int i = i1; i > 0; i += ((UndoTree.Node)nodes.get(i)).parent)
            depth1++;

        // Walk up the deeper side til they are at common depth...
        while (depth0 > depth1)
        {
            i0 += ((UndoTree.Node)nodes.get(i0)).parent;
            depth0--;
        }
        while (depth1 > depth0)
        {
            i1 += ((UndoTree.Node)nodes.get(i1)).parent;
            depth1--;
        }
        ...
        // This would get moved into UndoTree if UndoTree was to be made autonomous
    ...
    */
    ...
    public static class Snapshot
    {
        ...
        public int nOtherSquirrels = -1;
        public int otherSquirrelsCurrentNodes[] = null;
        public double otherSquirrelsCurrentNodeFractions[] = null;
    } // Snapshot
    ...
    public void takeSnapshot(Snapshot snapshot,
                             UndoTreeSquirrel otherSquirrels[])
    {
        for (int i = 0; i < otherSquirrels.length; ++i)
            if (otherSquirrels[i].model != model)
                throw new IllegalArgumentException("UndoTreeSquirrel.takeSnapshot called with other squirrel that's not in the same tree!");
        ...
        while (true)
        {
            ...
            int nOtherSquirrels = otherSquirrels.length;
            snapshot.nNodes = nNodes;
            snapshot.nOtherSquirrels = nOtherSquirrels;
            ...
            if (snapshot.otherSquirrelsCurrentNodes == null ||
                snapshot.otherSquirrelsCurrentNodes.length < nOtherSquirrels)
                snapshot.otherSquirrelsCurrentNodes = new int[nOtherSquirrels];
            if (snapshot.otherSquirrelsCurrentNodeFractions == null ||
                snapshot.otherSquirrelsCurrentNodeFractions.length < nOtherSquirrels)
                snapshot.otherSquirrelsCurrentNodeFractions = new double[nOtherSquirrels];
            ...
            synchronized(model.nodes) // model.nodes.size can change while waiting for this lock... and our currentNodeIndex might change with it due to our listener being called
            {
                ...
                for (int iNode = 0; iNode < nNodes; ++iNode)
                {
                    ...
                    PrivateNodeInfo privateNodeInfo = privateNodeInfosHashMap.get(node);
                ...
                for (int i = 0; i < nOtherSquirrels; ++i)
                {
                    snapshot.otherSquirrelsCurrentNodes[i] = otherSquirrels[i].currentNodeIndex;
                    snapshot.otherSquirrelsCurrentNodeFractions[i] = otherSquirrels[i].currentNodeFraction.get();
                }
                break;
        ...
        do { if (!(snapshot.currentNode >= 0)) throw new Error("CHECK failed at "+"com/donhatchsw/util/UndoTreeSquirrel.prejava"+"("+984 +"): " + "snapshot.currentNode >= 0" + ""); } while (false);
        do { if (!(snapshot.currentNode < snapshot.nNodes)) throw new Error("CHECK failed at "+"com/donhatchsw/util/UndoTreeSquirrel.prejava"+"("+985 +"): " + "snapshot.currentNode < snapshot.nNodes" + ""); } while (false);
    } // takeSnapshot
    ...
    * <pre>
    *   UndoTreeSquirrel squirrel = new UndoTreeSquirrel();
    *   squirrel.Do("A");
    *   squirrel.Do("B");
    *   squirrel.undo();
    *   squirrel.undo();
    *   squirrel.Do("C");
    *   squirrel.undo();
    *   squirrel.preferYoungerChild();
    *   squirrel.redo();
    *   squirrel.Do("D");
    *   squirrel.Do("E");
    *   System.out.println(squirrel.toStringVisualCool(true));
    *   System.out.println(squirrel.toStringVisualCool(false));
    * </pre>
    */
    ...
    public String toStringVisualCool(boolean stretchOutAlongTimeLine)
    {
        ...
        {
            UndoTree.Node node = (UndoTree.Node)model.nodes.get(iNode);
            PrivateNodeInfo privateNodeInfo = getOrMakePrivateNodeInfo(privateNodeInfosHashMap, node); // XXX shouldn't make it if it doesn't exist, that's lame
            if (node.children.length == 0)
            {
                                           ...
                                           double y[]) // output
    {
        ...
        for (int iChild = nNodes-1; iChild >= 0; iChild--)
        {
             ...
             && preferredChild[iChild] != currentNodeIndex)
            {
                ...
                if (false)
                {
                    ...
                    {System.out.print("        "); System.out.println("x.length" + " = " + (x.length));};
                    {System.out.print("        "); System.out.println("iChild" + " = " + (iChild));};
                    {System.out.print("        "); System.out.println("currentNodeIndex" + " = " + (currentNodeIndex));};
                }
                x[iChild] = ((1-(1.-currentNodeFrac))*(x[currentNodeIndex]) + (1.-currentNodeFrac)*(x[iChild]));
            }
        ...
        for (int iNode = 0; iNode < nNodes; ++iNode)
        {
            if (parents[iNode] == -1)
                ...
                y[iNode] = ((1-(stretchOutAlongTimeLine))*(y[parents[iNode]]) + (stretchOutAlongTimeLine)*(y[iNode-1])) + edgeLengths[iNode];
        }
    } // chooseNodePositions
    ...
    * The result can be turned back into another UndoTreeSquirrel
    ...
    */
    ...
    {
        System.out.println("in UndoTreeSquirrel.toString");
        if (!regex.matches(indentSpaces, "\\s*"))
            throw new IllegalArgumentException("UndoTreeSquirrel.toString given indent string "+Arrays.toStringCompact(indentSpaces)+", should be zero or more spaces");
        if (!regex.matches(separatorSpaces, "\\s+"))
            throw new IllegalArgumentException("UndoTreeSquirrel.toString given separator string "+Arrays.toStringCompact(separatorSpaces)+", should be one or more spaces");
        ...
        if (itemMatcher.reset("").matches())
            throw new IllegalArgumentException("UndoTreeSquirrel.toString given item regex "+Arrays.toStringCompact(itemRegex)+" which matches the empty string, that's messed up!");
        if (itemMatcher.reset("u").matches()
         || itemMatcher.reset("u1").matches()
         || itemMatcher.reset("u10").matches())
            throw new IllegalArgumentException("UndoTreeSquirrel.toString given item regex "+Arrays.toStringCompact(itemRegex)+" which matches something starting with \"u\", can't do that because it's special!");
        ...
        for (int iNode = 0; iNode < nNodes; ++iNode)
        {
            ...
            if (iNode != 0)
            {
                ...
                {
                    throw new IllegalArgumentException("UndoTreeSquirrel.toString given itemToString functor that produced string "+Arrays.toStringCompact(itemString)+" which does not match its regex pattern "+itemRegex+"!");
                }
                if (node.parent == -1)
            ...
            PrivateNodeInfo privateNodeInfo = getOrMakePrivateNodeInfo(privateNodeInfosHashMap, node);
        ...
        System.out.println("out UndoTreeSquirrel.toString");
        return s;
    } // toString

    /** Parses a string that was created by toString(). */
    public static UndoTreeSquirrel fromString(String s, ItemFromString itemFromString)
    {
        String itemRegex = itemFromString.regex();
        regex.Matcher itemMatcher = regex.Pattern.compile(itemRegex).matcher("dummy");
        // XXX dup code in the other place, should combine
        if (itemMatcher.reset("").matches())
            throw new IllegalArgumentException("UndoTreeSquirrel.fromString given item regex "+Arrays.toStringCompact(itemRegex)+" which matches the empty string, that's messed up!");
        if (itemMatcher.reset("u").matches()
         || itemMatcher.reset("u1").matches()
         || itemMatcher.reset("u10").matches())
            throw new IllegalArgumentException("UndoTreeSquirrel.fromString given item regex "+Arrays.toStringCompact(itemRegex)+" which matches something starting with \"u\", can't do that because it's special!");
        if (itemMatcher.reset("(0)").matches()
         || itemMatcher.reset("(1)").matches()
         || itemMatcher.reset("(10)").matches())
            throw new IllegalArgumentException("UndoTreeSquirrel.toString given item regex "+Arrays.toStringCompact(itemRegex)+" which matches a parenthesized number something like (2), can't do that because it's special!");
        ...
        if (s.length() < 2
         || s.charAt(0) != '{'
         || s.charAt(s.length()-1) != '}')
            throw new IllegalArgumentException("UndoTreeSquirrel.fromStringCompact called on string not surrounded by '{'");
        ...
        String words[];
        {
            ...
            else
            {
                ...
                do {if (true) throw new Error("Unimplemented at "+"com/donhatchsw/util/UndoTreeSquirrel.prejava"+"("+1501 +")"); } while (false); // XXX implement me!
        ...
        java.util.HashMap<UndoTree.Node,PrivateNodeInfo> privateNodeInfosHashMap = new java.util.HashMap<UndoTree.Node,PrivateNodeInfo>();

        UndoTree.Node nodes[] = new UndoTree.Node[Math.max(words.length,1)]; // at most this many
        com.donhatchsw.compat.ArrayList/*UndoTree.Node*/ childrens[] = new com.donhatchsw.compat.ArrayList[nodes.length];

        int nNodes = 0;
        nodes[nNodes++] = new UndoTree.Node(-1, null, currentTimeMillis);
        ...
        while (iWord < words.length)
        {
            ...
            {
                PrivateNodeInfo privateNodeInfo = getOrMakePrivateNodeInfo(privateNodeInfosHashMap, nodes[nNodes-1]);
              ...
              && words[iWord+2].equals("here)")))
            {
                if (currentNodeIndex != -1)
                    throw new IllegalArgumentException("UndoTreeSquirrel.fromString given an undo tree with more than one current node!");
            ...
            // etc.
            //
            if (regex.matches(words[iWord], "u\\d*")) // XXX precompile!
            {
                int parentOffset = -1 -
                ...
                if (nNodes-1+parentOffset < 0) // e.g. node 3 can have offset at most 2, which makes its parent node 0
                    throw new IllegalArgumentException("UndoTreeSquirrel.fromString node "+(nNodes-1)+"'s parent offset is "+parentOffset+", which would put the parent at "+(nNodes-1+parentOffset)+"");
                nodes[nNodes-1].parent = parentOffset;
                int parentIndex = nNodes-1+parentOffset;
                iWord++;
            }
            childrens[nNodes-1 + nodes[nNodes-1].parent].add(Integer.valueOf(-nodes[nNodes-1].parent));

            if (iWord == words.length)
                throw new IllegalArgumentException("UndoTreeSquirrel.fromString premature end of input trying to read item at word "+iWord+"");
            ...
            {
                {System.out.print("        "); System.out.println("words[iWord]" + " = " + (words[iWord]));};
                throw new IllegalArgumentException("UndoTreeSquirrel.fromString expected item at word "+iWord+" but got "+Arrays.toStringCompact(words[iWord])+" which does not match regex "+Arrays.toStringCompact(itemRegex)+"");
            }
        } // while (iWord < words.length)

        if (currentNodeIndex == -1)
            throw new IllegalArgumentException("UndoTreeSquirrel.fromString given an undo tree with no current node!");
        ...
        for (int iNode = 0; iNode < nNodes; ++iNode)
        {
            ...
            PrivateNodeInfo privateNodeInfo = privateNodeInfosHashMap.get(nodes[iNode]);
            if (privateNodeInfo != null
             && (privateNodeInfo.preferredChildIndex < -1 // -1 is always okay
              || privateNodeInfo.preferredChildIndex >= nodes[iNode].children.length))
            {
                throw new IllegalArgumentException("UndoTreeSquirrel.fromString node "+iNode+"'s preferred child is "+privateNodeInfo.preferredChildIndex+" but only has "+nodes[iNode].children.length+" children!");
            }
        }

        UndoTreeSquirrel squirrel = new UndoTreeSquirrel();
        squirrel.currentNodeIndex = currentNodeIndex;
        squirrel.privateNodeInfosHashMap = privateNodeInfosHashMap;
        squirrel.model.nodes = new com.donhatchsw.compat.ArrayList(nNodes);
        for (int iNode = 0; iNode < nNodes; ++iNode)
            squirrel.model.nodes.add(nodes[iNode]);
        ...
        private void checkSane()
        {
            for (int iNode = 0; iNode < model.nodes.size(); ++iNode)
            {
                ...
                {
                    int childIndex = iNode + node.children[iChild];
                    UndoTree.Node child = (UndoTree.Node)model.nodes.get(childIndex);
                    //System.out.println(""+iNode+"->"+childIndex);
                    do { if (!((childIndex+child.parent)==(iNode))) throw new Error("CHECK failed at "+"com/donhatchsw/util/UndoTreeSquirrel.prejava"+"("+1671 +"): (" + "childIndex+child.parent" + ")" + "==" + "(" + "iNode" + ") ("+(childIndex+child.parent)+" vs. "+(iNode)+")"); } while (false);
                }
            }
        }
    ...
    public static void main(String args[])
    {
        ...
        squirrel.Do("A");
        squirrel.Do("B");
        squirrel.setPrivateLabel("private_before_fork");
        squirrel.undo();
        squirrel.undo();
        squirrel.Do("C");
        squirrel.setPrivateLabel("shared_before_fork");
        squirrel.undo();
        squirrel.preferYoungerChild();
        squirrel.redo();
        squirrel.Do("D");
        squirrel.Do("E");
        UndoTreeSquirrel squirrel0 = squirrel;
        UndoTreeSquirrel squirrel1 = new UndoTreeSquirrel(squirrel);
        squirrel0.Do("F0");
        squirrel1.Do("G1");
        squirrel0.Do("H0");
        squirrel0.setPrivateLabel("0_private_after_fork");
        squirrel1.setPrivateLabel("1_private_after_fork");
        squirrel0.setSharedLabel("0_shared_after_fork");
        squirrel1.setSharedLabel("1_shared_after_fork");
        System.out.println("Tree 0:");
        System.out.println(squirrel0.toStringVisualCool(true));
        System.out.println(squirrel0.toStringVisualCool(false));
        System.out.println("Tree 1:");
        System.out.println(squirrel1.toStringVisualCool(true));
        System.out.println(squirrel1.toStringVisualCool(false));



        UndoTreeSquirrel.ItemToString itemToString = new UndoTreeSquirrel.ItemToString(){
            public String regex()
            {
                //return "-?\\d+"; // ints
                if (false)
        ...
        UndoTreeSquirrel.ItemFromString itemFromString = new UndoTreeSquirrel.ItemFromString(){
            public String regex()
            {
                // return "-?\\d+"; // ints
                if (false)
        ...
        String s0 = squirrel0.toString(itemToString, indentSpaces, separatorSpaces);
        String s1 = squirrel1.toString(itemToString, indentSpaces, separatorSpaces);

        UndoTreeSquirrel t0 = UndoTreeSquirrel.fromString(s0, itemFromString);
        UndoTreeSquirrel t1 = UndoTreeSquirrel.fromString(s1, itemFromString);
        ...
        {System.out.print("        "); System.out.println("s0" + " = " + (s0));};
        {System.out.print("        "); System.out.println("u0" + " = " + (u0));};
        do { if (!(u0.equals(s0))) throw new Error("CHECK failed at "+"com/donhatchsw/util/UndoTreeSquirrel.prejava"+"("+1766 +"): " + "u0.equals(s0)" + ""); } while (false);
        do { if (!(u1.equals(s1))) throw new Error("CHECK failed at "+"com/donhatchsw/util/UndoTreeSquirrel.prejava"+"("+1767 +"): " + "u1.equals(s1)" + ""); } while (false);

        // XXX maybe should compare the visualcools too?
    } // main
} // class UndoTreeSquirrel
